...
def talker():
    ...
    e = 0
    for time1 in range(0, len(database['Right']['u_trajs'])):#15, len(database['Right']['trajs'])):
        if(database['Right']['costs'][time1] > 0.01):
            ...
            weight_quad_zmp1 = np.array([2.0, 13.0]) ##11
            weight_quad_cam = np.array([0.05, 0.05])#([weight_quad_camy] + [weight_quad_camx])
            weight_quad_upper = np.array([0.01, 0.01])
            ...
            while booltemp == True:
                ...
                c_start = time.time()
                if iter_ == 0:
                    css = ddp.solve(xs, us, 2000)
                else:
                    css = ddp.solve(ddp.xs, ddp.us, 2000)
                c_end = time.time()
                ...
                if booltemp1 == False:
                    ...
                    with open('/home/jhk/ssd_mount/afterprocessing/upperbody/integral/filename4.pkl', 'wb') as f:
