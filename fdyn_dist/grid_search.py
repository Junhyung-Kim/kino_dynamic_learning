...
class CNN(nn.Module):
    def __init__(self, input_size, output_size, device):
        ...
        self.layer3 = torch.nn.Sequential(
            torch.nn.Linear(in_features = input_size, out_features= 100),
            torch.nn.ReLU(),
            torch.nn.Linear(in_features = 100, out_features = output_size)
            )
...
def define_RBF(dof=39, nbStates=60, offset=200, width=60, T=4000, coeff = 430):
    tList = np.arange(T)
...
def PCAlearning(time_step):
    ...
    file_name ='/home/jhk/'
    file_name2 = 'timestep=11_finish.txt'
    file_name3 = file_name  + file_name2
    ...
    #[1, 1, 3, 49, 69.57055002401981] [2, 1, 3, 49, 35.242546243343696]
    #[2, 2, 7, 40, 128.51960995860054]

    #[3, 1, 3, 60, 6.958509338214374, 0.0007638065941889904, 6.957342915494418, 0.0004026161257675362]
            ...
            for k in range(1,10):
                for f in range(40, 50):
                    ...
                    for num in range(0, 60):
                        aa_ = w_trajs[key][30000].reshape(rbf_num,-1)
                        bb_ = w_vel_trajs[key][30000].reshape(rbf_num,-1)
                        #dd_ = w_acc_trajs[key][30000].reshape(rbf_num,-1)
                        #cc_ = w_u_trajs[key][30000].reshape(rbf_num,-1)
                        ee_ = w_x_trajs[key][30000].reshape(rbf_num,-1)

                        if num  == 0:
                            traj_cost = np.linalg.norm(trajs[key][30000][num] - np.dot(Phi[num], aa_)) *np.linalg.norm(trajs[key][30000][num] - np.dot(Phi[num], aa_)) 
                            vel_cost = np.linalg.norm(vel_trajs[key][30000][num] - np.dot(Phi[num], bb_)) * np.linalg.norm(vel_trajs[key][30000][num] - np.dot(Phi[num], bb_))
                            x_cost =  np.linalg.norm(x_trajs[key][30000][num] - np.dot(Phi[num], ee_)) *np.linalg.norm(x_trajs[key][30000][num] - np.dot(Phi[num], ee_)) 
                        else:
                            traj_cost = traj_cost + np.linalg.norm(trajs[key][30000][num] - np.dot(Phi[num], aa_)) *np.linalg.norm(trajs[key][30000][num] - np.dot(Phi[num], aa_)) 
                            vel_cost = vel_cost + np.linalg.norm(vel_trajs[key][30000][num] - np.dot(Phi[num], bb_)) * np.linalg.norm(vel_trajs[key][30000][num] - np.dot(Phi[num], bb_))
                            x_cost = x_cost + np.linalg.norm(x_trajs[key][30000][num] - np.dot(Phi[num], ee_)) *np.linalg.norm(x_trajs[key][30000][num] - np.dot(Phi[num], ee_)) 
                    
                    cost = traj_cost + vel_cost + x_cost
                    if kks == False:
                        kks = True
                        cost_temp = cost
                        k2 = [i, j, k, f, cost, traj_cost, vel_cost, x_cost]
                        print("start")
                    if(cost < cost_temp):
                        print("i")
                        k2 = [i, j, k, f, cost]
                        print([i, j, k, f, cost, traj_cost, vel_cost, x_cost])
                        cost_temp = cost
                    else:
                        print("k")
                        print(k2)
...
def talker():
    ...
    for time_step in range(0, 1):
        ...
        for i in range(0, len(q)):    
            q[i] = xs_pca_test[i]
        for i in range(21, 21 + len(qdot)):    
            qdot[i-21] = xs_pca_test[i]
        ...
        ddp.th_stop = 0.00001
        c_start = time.time()
        css = ddp.solve(xs_pca, us_pca, 30, False, 0.01)
        c_end = time.time()
