...
def talker():
    ...
    weight_quad_zmp1 = np.array([3.0, 10.0]) ##10, 40
    weight_quad_zmp2 = np.array([3.0, 10.0]) ##11
    weight_quad_cam = np.array([0.01, 0.01])#([0.008, 0.008])([weight_quad_camy] + [weight_quad_camx])
    weight_quad_upper = np.array([50.0, 10.0])
    ...
    lb_ = np.ones([2, N])
    ub_ = np.ones([2, N])
    weight_quad_cp = np.array([50.0, 80.0])
    ...
    0.00000000e+00])
                ...
                c_start = time.time()
                css = ddp.solve(xs_pca, us_pca, 10, False, 0.0000008)
                c_end = time.time()
