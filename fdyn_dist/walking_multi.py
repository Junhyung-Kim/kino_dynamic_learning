...
def talker():
    ...
    for time_step in range(1, total_time):
        ...
        weight_quad_zmp1 = np.array([0.005, 0.005]) ##11
        weight_quad_cam = np.array([0.001, 0.001])#([weight_quad_camy] + [weight_quad_camx])
        weight_quad_upper = np.array([0.005, 0.005])
        weight_quad_com = np.array([20.0, 20.0, 1.0])#([weight_quad_comx] + [weight_quad_comy] + [weight_quad_comz])
        weight_quad_rf = np.array([10.0, 1.0, 10.0, 0.5, 0.5, 0.5])#np.array([weight_quad_rfx] + [weight_quad_rfy] + [weight_quad_rfz] + [weight_quad_rfroll] + [weight_quad_rfpitch] + [weight_quad_rfyaw])
        weight_quad_lf = np.array([10.0, 1.0, 10.0, 0.5, 0.5, 0.5])#np.array([weight_quad_lfx] + [weight_quad_lfy] + [weight_quad_lfz] + [weight_quad_lfroll] + [weight_quad_lfpitch] + [weight_quad_lfyaw])
