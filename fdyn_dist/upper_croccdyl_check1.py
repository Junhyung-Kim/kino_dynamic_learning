...
def talker():
    ...
    #with open('/home/jhk/walkingdata/beforedata/fdyn/timestep=12_finish/timestep=12_finish_', 'rb') as f:
        database = pickle.load(f,  encoding='iso-8859-1')
    f.close()
    ...
    if data_processing == True:
        ...
        for i in range(0, 1):#len(database[key]['trajs'])):
            ...
            u_temp = []
            for j in range(0, N):
                trajs_temp.append(database['Right']['trajs'][k1*j])
                x_temp.append(database['Right']['x_state'][k1*j])
                vel_temp.append(database['Right']['vel_trajs'][k1*j])
                if j != N-1:
                    acc_temp.append(database['Right']['acc_trajs'][k1*j])
                    u_temp.append(database['Right']['u_trajs'][k1*j])
                if j == N-1:
    ...
    #with open('/home/jhk/ssd_mount/beforedata/integrate_fdyn/filename3_40_-6775.pkl', 'rb') as f:
    
    with open('/home/jhk/ssd_mount/filename3.pkl', 'rb') as f:
    #file_name ='/home/jhk/ssd_mount/filenametest_'
    #file_name2 = '.pkl'
    #file_name3 = file_name + str(timestep) + file_name2
    #with open(file_name3, 'rb') as f:
        
        crocs_data = pickle.load(f,  encoding='iso-8859-1')
    f.close()
    ''''''
    for i in range(0, len(crocs_data['Right']['trajs'][0])):
        q[i] = crocs_data['Right']['trajs'][0][i]
    for i in range(0, len(crocs_data['Right']['vel_trajs'][0])):
        qdot[i] = crocs_data['Right']['vel_trajs'][0][i]
    ...
    time1d = 0 #len(crocs_data['Right']['x_state'])- 1
    for i in range(0, N-1):
        print(i)
        '''
        print( crocs_data['Right']['x_state'][0][i])
        '''
        a = [crocs_data['Right']['x_state'][i][2], crocs_data['Right']['x_state'][i][6]]
        b = [database['Right']['x_state'][i][2], database['Right']['x_state'][i][6]]
        #print("zmp")
        #print(a)
        #print(b)
        
        for j in range(0, len(crocs_data['Right']['trajs'][i])):
            q[j] = crocs_data['Right']['trajs'][i][j]
        for j in range(0, len(crocs_data['Right']['vel_trajs'][i])):
            qdot[j] = crocs_data['Right']['vel_trajs'][i][j]
        if i != 0:
            for j in range(0, len(crocs_data['Right']['u_trajs'][i])):
                qddot[j] = crocs_data['Right']['u_trajs'][i-1][j]
        else:
            for j in range(0, len(crocs_data['Right']['u_trajs'][i])):
                qddot[j] = 0.0
        pinocchio.forwardKinematics(model.model, data, q, qdot)
        ...
        a = [crocs_data['Right']['x_state'][i][0], crocs_data['Right']['x_state'][i][4], 0.81]
        b = [database['Right']['x_state'][i][0], database['Right']['x_state'][i][4], 0.81]
        
        print("com")
        print(a - data.com[0])
        
        print("cam")
        a = [crocs_data['Right']['x_state'][i][7], crocs_data['Right']['x_state'][i][3], 0.0]
        ...
        #print(RF_tran.translation)
        #print(array_boundRF_[i])
        #print(LF_tran.translation)
    ...
    for i in range(0, N):
        if (crocs_data['Right']['x_state'][i][2] > array_boundx_[i][0]) and (crocs_data['Right']['x_state'][i][2] < array_boundx_[i][1]):
            a1 = 0
        else:
            print("zmpx not ok")
            a = [crocs_data['Right']['x_state'][i][2], crocs_data['Right']['x_state'][i][3]]
            print(a[0])
            print(array_boundx_[i])
            print(i)

        if (crocs_data['Right']['x_state'][i][6] > array_boundy_[i][0]) and (crocs_data['Right']['x_state'][i][6] < array_boundy_[i][1]):
            a1 = 0
        else:
            print("zmpy not ok")
            a = [crocs_data['Right']['x_state'][i][6], crocs_data['Right']['x_state'][i][3]]
            print(a[0])
    ...
    for i in range(0, N):
        print(i)
        print(crocs_data['Right']['trajs'][i][19:])
        #print(crocs_data['Right']['trajs'][i][6])
       # print(crocs_data['Right']['x_state'][i][4])
        print([array_boundx_[i],array_boundy_[i]])
        print([crocs_data['Right']['x_state'][i][2], crocs_data['Right']['x_state'][i][6]])
    ...
    '''
    print("ddd")
    print(crocs_data['Right']['trajs'][0])
    print(crocs_data['Right']['trajs'][59])
    print(crocs_data['Right']['x_state'][59])

    #print(crocs_data['Right']['vel_trajs'][0])
    #print(crocs_data['Right']['u_trajs'][0])
    print(crocs_data['Right']['costs'])

    '''
    for i in range(0, N):
        print(i)
        print(crocs_data['Right']['x_state'][i])
    '''
