...
def talker():
    ...
    if data_processing == True:
        ...
        for i in range(0, 1):#len(database[key]['trajs'])):
            ...
            u_temp = []
            for j in range(0, N):
                trajs_temp.append(database['Right']['trajs'][i][j])
                x_temp.append(database['Right']['x_state'][i][j])
                vel_temp.append(database['Right']['vel_trajs'][i][j])
                if j != N-1:
                    acc_temp.append(database['Right']['acc_trajs'][i][j])
                    u_temp.append(database['Right']['u_trajs'][i][j])
                if j == N-1:
    ...
    print(crocs_data['Right']['trajs'][0][0])
