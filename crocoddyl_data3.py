...
def PCAlearning():
    ...
    with open('/home/jhk/ssd_mount/afterprocessing/3/Fdyn_data5.txt', 'rb') as f:
...
def talker():
    ...
    model = RobotWrapper.BuildFromURDF("/usr/local/lib/python3.8/dist-packages/robot_properties_tocabi/resources/urdf/tocabi.urdf","/home/jhk/catkin_ws/src/dyros_tocabi_v3/tocabi_description/meshes",pinocchio.JointModelFreeFlyer())  
    ...
    for time1 in range(40,len(database_['Right']['u_trajs'])):#15, len(database_['Right']['trajs'])):
        ...
        weight_quad_zmp1 = np.array([3.0, 7.0])
        weight_quad_cam = np.array([0.002, 0.002])#([weight_quad_camy] + [weight_quad_camx])
        weight_quad_com = np.array([5.0, 5.0, 2.0])#([weight_quad_comx] + [weight_quad_comy] + [weight_quad_comz])
        ...
        while booltemp == True:
            ...
            c_start = time.time()
            if iter_ == 0:
                css = ddp.solve(xs, us, 180000)
            else:
                ddp.th_stop = 0.0001
                css = ddp.solve(ddp.xs, ddp.us, 10000)
            c_end = time.time()
            ...
            if booltemp1 == False:
                ...
                with open('/home/jhk/ssd_mount/afterprocessing/3/filename1.pkl', 'wb') as f:
