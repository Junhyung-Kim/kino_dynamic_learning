...
def PCAlearning():
    ...
    with open('/home/jhk/ssd_mount/beforedata/ssp1/timestep=1/Fdyn_data5_8_7_0.002_9.txt', 'rb') as f:
...
def talker():
    ...
    for time1 in range(0, 1):# len(database_['Right']['trajs'])):
        ...
        for i in range(0,N-1):
            ...
            runningCostModel_vector[i].addCost("footReg1", foot_trackR[i], 3.0)
            runningCostModel_vector[i].addCost("footReg2", foot_trackL[i], 3.0)   
        ...
        terminalCostModel.addCost("footReg1", foot_trackR[N-1], 3.0)
        terminalCostModel.addCost("footReg2", foot_trackL[N-1], 3.0)
        ...
        while booltemp == True:
            ...
            c_start = time.time()
            if iter_ == 0:
                css = ddp.solve(xs, us, 20000)
            else:
                css = ddp.solve(ddp.xs, ddp.us, 20000)
            c_end = time.time()
            duration = (1e3 * (c_end - c_start))
            ...
            if booltemp1 == True:
                ...
                with open('/home/jhk/ssd_mount/filename.pkl', 'wb') as f:
