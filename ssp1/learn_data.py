...
class CNN(nn.Module):
    def __init__(self, input_size, output_size, device):
        ...
        self.layer3 = torch.nn.Sequential(
            torch.nn.Linear(in_features = input_size, out_features= int((input_size + output_size))),
            torch.nn.ReLU(),
            torch.nn.Linear(in_features = int((input_size + output_size)), out_features = output_size)
            )
