...
if __name__=='__main__':
    ...
    learn_type = 0
    array_ = ["timestep=12_finish", "timestep=18_finish", "timestep=31_finish",
"timestep=13_finish", "timestep=19_finish", "timestep=35_finish",
"timestep=14_finish", "timestep=20_finish", "timestep=37_finish",
"timestep=15_finish", "timestep=23_finish",        "timestep=38_finish",
"timestep=16_finish", "timestep=26_finish", "timestep=39_finish",
"timestep=17_finish", "timestep=30_finish", "timestep=40_finish",]
    
    for kk in range(0, len(array_)):
        database = dict()
        database['left'] = dict()
        database['Right'] = dict()
        ...
        for key in keys:
            d = np.array([])
            for i in range(x_inputs[key].shape[0]):
            ...
            d = d.reshape(x_inputs[key].shape[0], 43)
            x_inputs[key] = d
        ...
        while len(JJ) < 14:
            co = co + 1
            ran = np.random.randint(x_inputs[key].shape[0])
        ...
        print(best)
        filename = '/home/jhk/ssd_mount3/beforedata/ssp1/'
        filename2 = filename + array_[kk] + '/' + 'output.txt'
        np.savetxt(filename2, best)
        '''
        with open("/home/jhk/ssd_mount3/beforedata/ssp1/timestep=12_finish/output.txt", "w") as txt_file:
            for line in best:
