...
def PCAlearning():
    ...
    with open('/home/jhk/ssd_mount/beforedata/i=-1-3,j=0-10,2/', 'rb') as f:
...
def talker():
    ...
    for time_step in range(4, 5):
        ...
        #with open('/home/jhk/ssd_mount/afterdata/i=-1-3,j=0-10,2/Fdyn_data5.txt', 'rb') as f:
            database = pickle.load(f,  encoding='iso-8859-1')
        f.close()
        ...
        if data_processing == True:
            ...
            for i in range(0, len(database[key]['trajs'])):
                database_['Right']['x_inputs'].append(database['Right']['trajs'][i][0+time_step])
                trajs_temp = []
                acc_temp = []
                x_temp = []
                vel_temp = []
                u_temp = []
                for j in range(0, N):
                    trajs_temp.append(database['Right']['trajs'][i][k1*j+time_step])
                    x_temp.append(database['Right']['x_state'][i][k1*j+time_step])
                    vel_temp.append(database['Right']['vel_trajs'][i][k1*j+time_step])
                    if j != N-1:
                        acc_temp.append(database['Right']['acc_trajs'][i][k1*j+time_step])
                        u_temp.append(database['Right']['u_trajs'][i][k1*j+time_step])
                    if j == N-1:
        ...
        e = 0
        for time1 in range(0, 6775):#  len(database_['Right']['u_trajs'])):#15, len(database_['Right']['trajs'])):
            ...
            while booltemp == True:
                ...
                if iter_ % 2 == 0:
                    ...
                    if iter_ == 0:
                        print("c")
                        print(time1)
                        css = ddp.solve(xs, us, 1000, False, 0.005)
                else:
                    print("b")
                    css = ddp.solve(ddp.xs, ddp.us, 1000, False, 0.005)
                c_end = time.time()
                ...
                if booltemp1 == False:
                    if iter_ >= 5 or abs(ddp.cost - cost_prev) < 0.0000001 and css == True:
                        booltemp = False
                    cost_prev = ddp.cost
                    if iter_ == 0:
                        for key in crocs_data.keys():
                        ...
                        crocs_data['Right']['time'].append(time_step*0.02)
                    else:
                        traj = np.array(ddp.xs)[:,0:21]
                        vel_traj = np.array(ddp.xs)[:,21:41]
                        x_traj = np.array(ddp.xs)[:, 41:49]
                        ...
                        crocs_data['Right']['time'][e] = (time_step*0.02)
                        #database_[key]['data_phases_set'].append(css)
                    print(time1)
                    print(ddp.xs[0][0:21])
                    print(ddp.xs[0][41:49])
                    print(ddp.xs[N-1][0:21])
                iter_ = iter_ + 1
                if booltemp == False:
                    e = e+1
            if time1 % 1000 == 0:
                file_name ='/home/jhk/ssd_mount/beforedata/integrate_fdyn/filename3_'
                file_name2 = '_-6775.pkl'
