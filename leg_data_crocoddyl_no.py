...
def talker():
    ...
    print("start")
    for step_time in range(0, 2):
        f = open("/home/jhk/ssd_mount2/beforedata/fdyn_int/timestep=15/lfoot1_ssp1_1.txt", 'r')
        f1 = open("/home/jhk/ssd_mount2/beforedata/fdyn_int/timestep=15/rfoot1_ssp1_1.txt", 'r')
        f2 = open("/home/jhk/ssd_mount2/beforedata/fdyn_int/timestep=15/zmp2_ssp1_1.txt", 'r')
        ...
        #ilename1 = str('/')
        filename2 = '/i=10/Fdyn_data7.txt'
        filename3 = '/home/jhk/ssd_mount2/beforedata/ssp1/Fdyn_data7_test.txt'
        ...
        e = 0
        for time1 in range(0, 1):#len(database_['Right']['trajs'])):#15, len(database_['Right']['trajs'])):
            ...
            '''
            weight_quad_zmp = np.array([2.0, 100.0])#([weight_quad_zmpx] + [weight_quad_zmpy])
            weight_quad_zmp1 = np.array([0.5, 10.0]) ##11
            weight_quad_cam = np.array([0.1, 0.1])#([weight_quad_camy] + [weight_quad_camx])
            ...
            weight_quad_rf = np.array([0.3, 1.0, 0.3, 0.1, 0.1, 0.1])#np.array([weight_quad_rfx] + [weight_quad_rfy] + [weight_quad_rfz] + [weight_quad_rfroll] + [weight_quad_rfpitch] + [weight_quad_rfyaw])
            weight_quad_lf = np.array([0.3, 1.0, 0.3, 0.1, 0.1, 0.1])#np.array([weight_quad_lfx] + [weight_quad_lfy] + [weight_quad_lfz] + [weight_quad_lfroll] + [weight_quad_lfpitch] + [weight_quad_lfyaw])
            ...
            while booltemp == True:
                ...
                if iter_ % 2 == 0:
                    ddp.th_stop = 0.000000001
                    print("a")
                    if iter_ == 0:
                        #css = ddp.solve(xs, us, 3000, False)
                        c_start = time.time()
                        css = ddp.solve(xs, us, 300, False, 0.001)
                        #problemWithRK4.calc(xs,us) #3.33, 0.88
                        #problemWithRK4.calcDiff(xs,us)
                        c_end = time.time()
                else:
                    print("b")
                    #css = ddp.solve(ddp.xs, ddp.us, 5000, False, 0.01)
            ...
            if time1 % 5 == 0:
                filename = '/home/jhk/ssd_mount/beforedata/filename3_'
                filename1 = 'test1'
                filename2 = '.pkl'
