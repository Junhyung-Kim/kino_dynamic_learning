...
def PCAlearning():
    ...
    with open('/home/jhk/ssd_mount/beforedata/ssp2/i=3,j2,6,9/Fdyn_data7.txt', 'rb') as f:
...
def talker():
    ...
    for i in range(0, N):
        if i == 0:
            ...
            array_boundx_[i] = array_boundx[k3*(i-1)]
            array_boundy_[i] = array_boundy[k3*(i-1)]
    ...
    for i in range(0, N):
        if i == 0:
            ...
            zmp_refx_[i] = zmp_refx[k*(i-1)]
            zmp_refy_[i] = zmp_refy[k*(i-1)]
    f.close()
    f1.close()
    f2.close()
    ...
    e = 0
    for time1 in range(0, len(database_['Right']['u_trajs'])):#15, len(database_['Right']['trajs'])):
        ...
        while booltemp == True:
            ...
            if iter_ % 2 == 0:
                ...
                print("a")
                if iter_ == 0:
                    css = ddp.solve(xs, us, 20000, False, 0.1)
            else:
                print("b")
                css = ddp.solve(ddp.xs, ddp.us, 20000, False, 0.1)
            c_end = time.time()
            #ddp.xs = xs
            duration = (1e3 * (c_end - c_start))
            ...
            if booltemp1 == False:
                ...
                with open('/home/jhk/ssd_mount/beforedata/ssp2/i=3,j2,6,9/filename3.pkl', 'wb') as f:
        ...
        #print(crocs_data['Right']['x_state'][time1d][0])
        #print(crocs_data['Right']['acc_trajs'][time1d][0])
        #print(crocs_data['Right']['x_state'][time1d][1])
