...
def talker():
    ...
    for step_time in range(0, 1):
        ...
        for i in range(0, N):
            if i == 0:
                array_boundRF_[i] = np.sum([array_boundRF[k*i + step_time], [-0.03, 0.0, 0.15842]], axis = 0)
            else:
                array_boundRF_[i] = np.sum([array_boundRF[k*(i-1) + step_time], [-0.03, 0.0, 0.15842]], axis = 0)
        ...
        for i in range(0, N):
            if i == 0:
                array_boundLF_[i] = np.sum([array_boundLF[k*i + step_time], [-0.03, 0.0, 0.15842]], axis = 0)
            else:
                array_boundLF_[i] = np.sum([array_boundLF[k*(i-1) + step_time], [-0.03, 0.0, 0.15842]], axis = 0)
        ...
        for i in range(0, N):
            if i == 0:
                ...
                array_boundx_[i] = array_boundx[k3*(i-1) + step_time]
                array_boundy_[i] = array_boundy[k3*(i-1) + step_time]
        ...
        filename = '/home/jhk/ssd_mount/beforedata/ssp1'
        #ilename1 = str('/')
        filename2 = '/i=9/Fdyn_data8.txt'
        ...
        if data_processing == True:
            ...
            for i in range(0, len(database[key]['trajs'])):
                if (len(database['Right']['trajs'][i]) > 90):
                    ...
                    for j in range(0, N):
                        ...
                        if j != N-1:
                            ...
                            u_temp.append(database['Right']['u_trajs'][i][k1*j + step_time + 1])
                        if j == N-1:
        ...
        for time1 in range(0, len(database_['Right']['trajs'])):#15, len(database_['Right']['trajs'])):
            ...
            runningConstraintModel_vector = [None] * (N-1)
            ...
            for i in range(0,N-1):
                ...
                camBoundCost_vector[i] = crocoddyl.ConstraintModelResidual(state_vector[i], crocoddyl.ResidualModelCentroidalAngularMomentum(state_vector[i], actuation_vector[i].nu + 4))
                comBoundCost_vector[i] = crocoddyl.ConstraintModelResidual(state_vector[i], crocoddyl.ResidualModelCoMKinoPosition(state_vector[i], np.array([0.0, 0.0, data.com[0][2]]), actuation_vector[i].nu + 4))
                ...
                runningConstraintModel_vector[i] = crocoddyl.ConstraintModelManager(state_vector[i], actuation_vector[i].nu + 4)
                ...
                #runningConstraintModel_vector[i].addConstraint("comReg", comBoundCost_vector[i])#, 1.0)
                runningConstraintModel_vector[i].addConstraint("camReg", camBoundCost_vector[i])#, 1.0)
                runningCostModel_vector[i].addCost("footReg1", foot_trackR[i], 1.0)
                runningCostModel_vector[i].addCost("footReg2", foot_trackL[i], 1.0)
               
                runningDAM_vector[i] = crocoddyl.DifferentialActionModelKinoDynamics(state_vector[i], actuation_vector[i], runningCostModel_vector[i],  runningConstraintModel_vector[i])
            ...
            camBoundCost_vector[N-1] = crocoddyl.ConstraintModelResidual(state_vector[N-1], crocoddyl.ResidualModelCentroidalAngularMomentum(state_vector[N-1], actuation_vector[N-1].nu + 4))
            comBoundCost_vector[N-1] = crocoddyl.ConstraintModelResidual(state_vector[i], crocoddyl.ResidualModelCoMKinoPosition(state_vector[i], np.array([0.0, 0.0, data.com[0][2]]), actuation_vector[i].nu + 4))
            ...
            terminalConstraintModel = crocoddyl.ConstraintModelManager(state_vector[N-1], actuation_vector[N-1].nu + 4)
            ...
            #terminalConstraintModel.addConstraint("comReg", comBoundCost_vector[N-1])#, 1.0)
            ...
            terminalDAM = crocoddyl.DifferentialActionModelKinoDynamics(state_vector[N-1], actuation_vector[N-1], terminalCostModel, terminalConstraintModel)
            ...
            ddp = crocoddyl.SolverIntro(problemWithRK4)
            ...
            while booltemp == True:
                ...
                if iter_ % 2 == 0:
                    ddp.th_stop = 0.00000002
                    print("a")
                    if iter_ == 0:
                        #css = ddp.solve(xs, us, 3000, False)
                        c_start = time.time()
                        css = ddp.solve(xs, us, 1000, False)#, True)
                ...
                iter_ = iter_ + 1
            if time1 % 30 == 0:
                filename = '/home/jhk/ssd_mount/beforedata/ssp1/i=9/filename3_'
