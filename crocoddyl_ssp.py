...
def talker():
    ...
    #/home/jhk/ssd_mount/beforeprocessing/SSP/i=0-8,j=13/Fdyn_data17.txt'
    ...
    for time1 in range(0, len(database_['Right']['u_trajs'])):#15, len(database_['Right']['trajs'])):
        ...
        while booltemp == True:
            ...
            if iter_ % 2 == 0:
                ...
                print("a")
                if iter_ == 0:
                    css = ddp.solve(xs, us, 1000, False, 0.0009)
            else:
                print("b")
                css = ddp.solve(ddp.xs, ddp.us, 1000, False, 0.0009)
            c_end = time.time()
            #ddp.xs = xs
            duration = (1e3 * (c_end - c_start))
