...
class AE(models.Model):                                       # AE 모델링 수행하는 역할
    def __init__(self, x_nodes, z_dim):                # x_nodes : 입력 노드수, z_dim : 은닉 노드 수
        ...
        '''                 # 입력 계층
        z = layers.Dense(int(z_dim* 18))(x)         # 은닉 계층 z_dim에 따라 노드수 정해짐, 활성화 함수는 'relu'
        z = layers.Dense(int(z_dim* 15))(z)   
        z = layers.Dense(int(z_dim* 12))(z) 
        z = layers.Dense(int(z_dim* 9))(z)   #
        z = layers.Dense(int(z_dim* 6))(z)
        z = layers.Dense(int(z_dim* 3))(z)
        z = layers.Dense(int(z_dim* 1))(z) 

        y = layers.Dense(int(z_dim*3))(z) 
        y = layers.Dense(int(z_dim*6))(y) 
        y = layers.Dense(int(z_dim*9))(y) 
        y = layers.Dense(int(z_dim*12))(y) 
        y = layers.Dense(int(z_dim*15))(y) 
        y = layers.Dense(int(z_dim*18))(y)
        y = layers.Dense(x_nodes)(y) 
        '''
        
        '''
        x = layers.Input(shape=x_shape)  
        z = layers.Dense(int(z_dim* 7.5))(x)         # 은닉 계층 z_dim에 따라 노드수 정해짐, 활성화 함수는 'relu'
        z = layers.Dense(int(z_dim* 6))(z)   
        z = layers.Dense(int(z_dim* 6.5))(z) 
        z = layers.Dense(int(z_dim* 6))(z)   #
        z = layers.Dense(int(z_dim* 5.5))(z)
        z = layers.Dense(int(z_dim* 5))(z)
        z = layers.Dense(int(z_dim* 4.5))(z)
        z = layers.Dense(int(z_dim* 4))(z)
        z = layers.Dense(int(z_dim* 3.5))(z)
        z = layers.Dense(int(z_dim* 3))(z)
        z = layers.Dense(int(z_dim* 2.5))(z)
        z = layers.Dense(int(z_dim* 2))(z)
        z = layers.Dense(int(z_dim* 1.5))(z)
        z = layers.Dense(int(z_dim* 1))(z) 
        #activation='gelu'
        y = layers.Dense(int(z_dim*1.5))(z) 
        y = layers.Dense(int(z_dim*2))(y) 
        y = layers.Dense(int(z_dim*2.5))(y) 
        y = layers.Dense(int(z_dim*3))(y) 
        y = layers.Dense(int(z_dim*3.5))(y) 
        y = layers.Dense(int(z_dim*4))(y) 
        y = layers.Dense(int(z_dim*4.5))(y) 
        y = layers.Dense(int(z_dim*5))(y) 
        y = layers.Dense(int(z_dim*5.5))(y) 
        y = layers.Dense(int(z_dim*6))(y)
        y = layers.Dense(int(z_dim*6.5))(y)
        y = layers.Dense(int(z_dim*7))(y)
        y = layers.Dense(int(z_dim*7.5))(y)
        y = layers.Dense(x_nodes)(y) 
        '''

        x = layers.Input(shape=x_shape)     
        #z = layers.Dense(int(z_dim* 12), activation='gelu')(x)
        z = layers.Dense(int(z_dim* 10))(x)         # 은닉 계층 z_dim에 따라 노드수 정해짐, 활성화 함수는 'relu'
        z = layers.Dense(int(z_dim* 8))(z)
        z = layers.Dense(int(z_dim* 4))(z)
        z = layers.Dense(int(z_dim* 2), activation='gelu')(z)
        z = layers.Dense(int(z_dim* 1))(z)  
        
        #y = layers.Dense(int(z_dim *3))(z) 
        y = layers.Dense(int(z_dim*2), activation='gelu')(z) 
        y = layers.Dense(int(z_dim*4))(y) 
        y = layers.Dense(int(z_dim*8))(y) 
        y = layers.Dense(int(z_dim*10))(y) 
        #y = layers.Dense(int(z_dim*12), activation='gelu')(y)
        y = layers.Dense(x_nodes)(y)
        ...
        # AE 구조를 확정하고 컴파일 진행
        self.compile(optimizer = keras.optimizers.Adam(learning_rate=0.00004),
                     #optimizer='adadelta',        # 최적화 함수 : adadelta, (+ 모멘텀(가중치를 적절히 올리는 방식) = adam)
                     loss = tf.keras.losses.MeanSquaredError(), #'logcosh',
    ...
    def Encoder(self):
        return tf.keras.models.Model(self.x, self.z)
    ...
    def Decoder(self):
        ...
        '''
        y_layer = self.layers[-14]  
        y_layer1 = self.layers[-13]  
        y_layer2 = self.layers[-12]  
        y_layer3 = self.layers[-11]  
        y_layer4= self.layers[-10]
        y_layer5 = self.layers[-9]
        y_layer6 = self.layers[-8] 
        y_layer7 = self.layers[-7]  
        
        y_layer11= self.layers[-3]
        '''
       # y_layer8 = self.layers[-6]  
        y_layer9 = self.layers[-5]  
        y_layer10 = self.layers[-4]  
        y_layer11= self.layers[-3]
        y_layer12 = self.layers[-2]
        y_layer13 = self.layers[-1]           # 기존 은닉 계층을 사용하기에 self 사용, layers[-1]는 출력 계층을 의미
        '''
        y = y_layer(z)
        y = y_layer1(y)
        y = y_layer2(y)
        ...
        y = y_layer12(y)
        y = y_layer13(y)
        return tf.keras.models.Model(z, y)
...
class AE(torch.nn.Module):
    def __init__(self, input_size, output_size, device):
        ...
        self.encoder = torch.nn.Sequential(
            torch.nn.Linear(input_size * output_size, int(input_size * output_size/6)),
            torch.nn.ReLU(),
            torch.nn.Linear(int(input_size * output_size/6), int(input_size * output_size/12)),
            torch.nn.ReLU(),
            torch.nn.Linear(int(input_size * output_size/12), output_size)
        )
         
        self.decoder = torch.nn.Sequential(
            torch.nn.Linear(output_size, int(input_size * output_size/12)),
            torch.nn.ReLU(),
            torch.nn.Linear(int(input_size * output_size/12), int(input_size * output_size/6)),
            torch.nn.ReLU(),
            torch.nn.Linear(int(input_size * output_size/6), int(input_size * output_size)),
        )
...
def PCAlearning():
    ...
    global us_pca
    #sssss
    learn_type = 1  # dataload network 1
    ...
    if learn_type1 == 0:
        for key in keys:
            ...
            for key in keys:
                ...
                y_u_train[key] = y_x_train[key]
                w_trajs[key] = apply_RBF(y_train[key], Phi)
                w_trajs_test[key] = apply_RBF(y_test[key], Phi)
        ...
        y_u_test= torch.load( '/home/jhk/kino_dynamic_learning/dataset/y_u_test_train.pt')
        ...
        y_u_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset/y_u_train_train.pt')
        ...
        if load_learning == 0:
            if (learn_type2 == 0):
                ...
                for key in keys:
                    ...
                    pca[key] = AE(8*60, rbf_num)
                    
                    encoder = pca[key].Encoder()
                    decoder = pca[key].Decoder()
                    y_x_train[key] = y_x_train[key].reshape(10017, 8*60)
                    y_x_test[key] = y_x_test[key].reshape(1113, 8*60)
                    history = pca[key].fit(y_x_train[key].detach().numpy(), y_x_train[key].detach().numpy(),       # 입력, 출력을 동일하게 설정
                              epochs=10,             # 10회 학습
                    ...
                    for data in train_x_loader:
                        a = a + 1
                        seq, target = data
                        data = target.detach().numpy().reshape(8*rbf_num) 
                        w_x_trajs_pca[key].append(pca_x[key].Encoder(data).detach().numpy())
                    '''
                    pca[key].save('/home/jhk/kino_dynamic_learning/dataset/w_trajs_kernelpca_train.pkl')
                    ...
                    for data in train_x_loader:
                        ...
                        k = encoder.predict(data)
                        k1 = np.reshape(k,(1,1,36))
                        J = decoder.predict(k1)
                        J1 = pca[key].predict(data)
                        print(np.linalg.norm(J-J1))
                        print(np.linalg.norm(data-J1))
                        w_trajs_pca[key].append(k)
                        a = a + 1
                    w_trajs_pca[key] = np.array(w_trajs_pca[key])
                    '''
                    '''
                    for data in train_x_loader:
                        #a = a + 1
                        ...
                        print("Ddd")
                        print(np.linalg.norm(data-J1))
                        print(keras.losses.mean_absolute_error(data, J1))
                        #w_x_trajs_pca[key].append(k)
                    ...
                    for data in test_x_loader:
                        #a = a + 1
                        seq, target = data
                        data = target.detach().numpy().reshape(1, 8*60) 
                        #v_pred = pca[key].predict(data) 
                        #norm_eval = (v_pred.reshape(rbf_num,21)- target.detach().numpy().reshape(rbf_num,21))#.detach().numpy()
                        k = encoder.predict(data)
                        J1 = pca[key].predict(data)
                        #print("Ddd")
                        #print(np.linalg.norm(data-J1))
                        #print(keras.losses.mean_squared_error(data, J1))
                        
                        norm_eval = J1.reshape(60,8) - data.reshape(60,8)#y_u_test[key].detach().numpy()[JJ]
                        a3_1 = []
                        a3_2 = []
                        a = a + 1
                        for i in range(0, 60):
            ...
            if (learn_type2 == 0):     
                ...
                pca = keras.models.load_model('/home/jhk/kino_dynamic_learning/dataset/w_trajs_kernelpca_train.pkl', compile = True)
    ...
    if(learn_type3 == 0):
        if test_data == 0:
            for JJ in range(0, len(x_inputs_test[key])):
                ...
                if (learn_type2 == 0):
                    ...
                    w_traj = w_traj[0].detach().numpy()#(y_x_test[key].detach().numpy())[JJ]
                    #np.reshape(w_traj,(1, 8* rbf_num))
                    #print(np.shape(w_traj))
                    #w_traj = np.reshape(w_traj, (1, rbf_num * 8))
                    #print(np.shape(w_traj))
                   
                    #w_traj = encoder.predict(w_traj)
                    w_traj = np.reshape(w_traj, (1, 1, rbf_num))
                    w_traj = decoder.predict(w_traj)
                    w_traj = w_traj.reshape(60,8)
                elif (learn_type2 == 2):
                    w_traj = w_traj[0].detach().numpy()
                    w_traj = pca['Right'].inverse_transform(w_traj)[0]
                    w_traj = w_traj.reshape(rbf_num,-1)
                traj1 = w_traj.flatten() #np.dot(Phi,w_traj)
                ...
                norm_eval = traj1 - y_x_test[key].detach().numpy()[JJ].ravel().reshape(60,8)#y_u_test[key].detach().numpy()[JJ]
                a3_1 = []
                a3_2 = []
                for i in range(0, 60):
                    a3_1.append([norm_eval[i][0], norm_eval[i][1], norm_eval[i][2], norm_eval[i][4], norm_eval[i][5], norm_eval[i][6]])
                ...
                #a1 = np.linalg.norm((q_traj.ravel() - y_x_test[key].detach().numpy()[JJ].ravel()))
        ...
        if test_data == 0:
            for JJ in range(0, len(x_inputs_train[key])):
