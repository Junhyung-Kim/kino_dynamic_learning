...
class RNN_1TM(nn.Module):
    def __init__(self, input_dim, seq_length, hidden_dim, 
        ...
        self.layer_dim = 3
        if type_rnn == "GRU":
            self.GRU_enc = nn.GRU(input_size=self._input_dim, hidden_size=self._input_dim, num_layers=self.layer_dim, batch_first=True)
            self.GRU_dec = nn.GRU(input_size=self._input_dim, hidden_size= self._hidden_dim, num_layers=self.layer_dim, batch_first=True)
        elif type_rnn == "LSTM":
            self.GRU_enc = nn.LSTM(input_size=self._input_dim, hidden_size=self._input_dim, num_layers=self.layer_dim, batch_first=True)
            self.GRU_dec = nn.LSTM(input_size=self._input_dim, hidden_size=self._hidden_dim, num_layers=self.layer_dim, batch_first=True)
        self.fc = nn.Linear(input_dim, hidden_dim)
        self.fc1= nn.PReLU()
        self.embedding = nn.Embedding(self._hidden_dim, self._input_dim)
    ...
    def enc(self, x):
        ...
        h0 = torch.zeros(self.layer_dim, x.shape[0], self._input_dim)
        c0 = torch.zeros(self.layer_dim, x.shape[0], self._input_dim)
        
        if(self._type_rnn == "LSTM"):
            for _ in range(self._seq_length ):
                out, (h0, c0) = self.GRU_enc(x, (h0, c0))
        else:
            for _ in range(self._seq_length ):
                out, h0 = self.GRU_enc(x, h0)
    ...
    def dec(self, x):
        ...
        c = torch.zeros(self.layer_dim, x.shape[0], self._hidden_dim)
        #output = F.relu(x)
        outputs = []
        if(self._type_rnn == "LSTM"):
            for _ in range(self._seq_length ):
                out, (h, c) = self.GRU_dec(x, (h, c))
                outputs.append(out)
        else:
            for _ in range(self._seq_length ):
        ...
        outputs = torch.reshape(outputs,(1,60,self._hidden_dim))
...
def PCAlearning():
    ...
    if learn_type1 == 0:
        for key in keys:
            ...
            torch.save(x_inputs_train, '/home/jhk/kino_dynamic_learning/dataset_ete/x_inputs_train_train_ete.pt')
            torch.save(x_inputs_test, '/home/jhk/kino_dynamic_learning/dataset_ete/x_inputs_test_train_ete.pt')
            torch.save(y_test, '/home/jhk/kino_dynamic_learning/dataset_ete/y_test_train_ete.pt')
            torch.save(y_vel_test, '/home/jhk/kino_dynamic_learning/dataset_ete/y_vel_test_train_ete.pt')
            torch.save(y_u_test, '/home/jhk/kino_dynamic_learning/dataset_ete/y_u_test_train_ete.pt')
            torch.save(y_acc_test, '/home/jhk/kino_dynamic_learning/dataset_ete/y_acc_test_train_ete.pt')
            torch.save(y_x_test, '/home/jhk/kino_dynamic_learning/dataset_ete/y_x_test_train_ete.pt')
            torch.save(y_train, '/home/jhk/kino_dynamic_learning/dataset_ete/y_train_train_ete.pt')
            torch.save(y_vel_train, '/home/jhk/kino_dynamic_learning/dataset_ete/y_vel_train_train_ete.pt')
            torch.save(y_u_train, '/home/jhk/kino_dynamic_learning/dataset_ete/y_u_train_train_ete.pt')
            torch.save(y_acc_train, '/home/jhk/kino_dynamic_learning/dataset_ete/y_acc_train_train_ete.pt')
            torch.save(y_x_train, '/home/jhk/kino_dynamic_learning/dataset_ete/y_x_train_train_ete.pt')

        print("transform SAVE")
    else:
        x_inputs_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/x_inputs_train_train_ete.pt')
        x_inputs_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/x_inputs_test_train_ete.pt')
        y_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/y_test_train_ete.pt')
        y_vel_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/y_vel_test_train_ete.pt')
        #y_u_test= torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/y_u_test.pt')
        #y_acc_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/y_acc_test.pt')
        y_x_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/y_x_test_train_ete.pt')
        y_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/y_train_train_ete.pt')
        y_vel_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/y_vel_train_train_ete.pt')
        #y_u_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/y_u_train.pt')
        #y_acc_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/y_acc_train.pt')
        y_x_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset_ete/y_x_train_train_ete.pt')
    ...
    if(learn_type2 == 0):
        ...
        input_size = 23
        model = RNN_1TM(input_dim=input_size, seq_length = 60,
                    hidden_dim = 21, type_rnn = "LSTM",
                    device=device).to(device)
        
        #qdot
        model1 = RNN_1TM(input_dim=input_size, seq_length = 60,
                    hidden_dim = 20, type_rnn = "LSTM",
                    device=device).to(device)

        #x
        model2 = RNN_1TM(input_dim=input_size, seq_length = 60,
                    hidden_dim = 8, type_rnn = "LSTM",
                    device=device).to(device)
    ...
    if learn_type == 0:
        ...
        if(learn_type2 == 0):
            ...
            model2.load_state_dict(torch.load('/home/jhk/ssd_mount/GRU_train2.pkl'))
            error_ = []
            for data in test_x_loader:
                seq, target = data
                X = seq.reshape(batch_size, input_size).to(device)
                out = model2(X)
                norm_eval = (out - target).detach().numpy()
                a312 = []
                for i in range(0, 60):
                    a312.append([norm_eval[0][i][0], norm_eval[0][i][1], norm_eval[0][i][2], norm_eval[0][i][4], norm_eval[0][i][5], norm_eval[0][i][6]])
                a3123_ = np.linalg.norm(a312)
                error_.append(a3123_)
                
                    
            print(np.mean(error_, axis=0))
            '''    
            torch.save(model.state_dict(), '/home/jhk/ssd_mount/GRU_train.pkl')
            torch.save(model1.state_dict(), '/home/jhk/ssd_mount/GRU_train1.pkl')
            torch.save(model2.state_dict(), '/home/jhk/ssd_mount/GRU_train2.pkl')     
        elif(learn_type2 == 1): 
            criterion = nn.MSELoss()
            lr = 0.001
            num_epochs = 2
            ...
            for epoch in range(num_epochs):
                ...
                torch.save(model.state_dict(), '/home/jhk/ssd_mount/LSTM_train.pkl')
                torch.save(model1.state_dict(), '/home/jhk/ssd_mount/LSTM_train1.pkl')
                torch.save(model2.state_dict(), '/home/jhk/ssd_mount/LSTM_train2.pkl')           
    else:
        if(learn_type2 == 0):
            model.load_state_dict(torch.load('/home/jhk/ssd_mount/GRU_train.pkl'))
            model1.load_state_dict(torch.load('/home/jhk/ssd_mount/GRU_train1.pkl'))
            model2.load_state_dict(torch.load('/home/jhk/ssd_mount/GRU_train2.pkl'))
        elif(learn_type2 == 1):
            model.load_state_dict(torch.load('/home/jhk/ssd_mount/LSTM_train.pkl'))
            model1.load_state_dict(torch.load('/home/jhk/ssd_mount/LSTM_train1.pkl'))
            model2.load_state_dict(torch.load('/home/jhk/ssd_mount/LSTM_train2.pkl'))
    ...
    if(learn_type2 == 0):
        if test_data == 0:
            for JJ in range(0, len(x_inputs_test[key])):
                ...
                print("Sss")
                print(q_traj.ravel()-trajs[key][KKK].ravel())
                #print(trajs[key][KKK].ravel())
                print(X)
                print(a1)
                print(np.linalg.norm(q_traj.ravel()))

                np.save('data_GRU_train.npy', np.array(error))
                #np.save('data_GRU_train1.npy', np.array(time_))
        else:
        ...
        if test_data == 0:
            for JJ in range(0, len(x_inputs_test[key])):
                ...
                np.save('data_LSTM_train.npy', np.array(error))
                np.save('data_LSTM_train1.npy', np.array(time_))
        else:
            JJ = np.random.randint(x_inputs_test[key].shape[0])
            X = x_inputs_test['Right'][JJ][None,:]
            X = X.reshape(1, 1, input_size).to(device)   
