...
def PCAlearning(time_step):
    ...
    file_name ='/home/jhk/ssd_mount/beforedata/ssp1/dist/time_step=0/-0.004'
    file_name2 = '/filename3_9_0.pkl'
    ...
    for key in keys:
        ...
        for i in range(0, num_desired):
            c = np.append(np.append(x_inputs[key][i], vel_trajs[key][i][0], axis=0), np.array([x_trajs[key][i][0][2], x_trajs[key][i][0][6]]), axis=0)
    ...
    timestep = 60
    rbf_num = 49
    Phi = define_RBF(dof=19, nbStates =rbf_num, offset = 1, width = 1, T = timestep, coeff =3)
    ...
    if learn_type1 == 0:
        ...
        for key in keys:
            ...
            loss = nn.MSELoss()
            '''
            print(pca[key].explained_variance_ratio_)
            print(pca_x[key].explained_variance_ratio_)
            print(pca_vel[key].explained_variance_ratio_)
            for epoch in range(num_epochs):
                ...
                for i in range(0,len(w_trajs_pca[key])): 
                    ...
                    #batch_loss = loss(vel_trajs[key][i].flatten(), torch.FloatTensor(v_pred.flatten())) # difference between actual and reconstructed   
                    #losses.append(batch_loss.item())
                    #running_loss = np.mean(losses)
                    running_loss = numpy.linalg.norm(np.subtract(trajs[key][i].flatten(),v_pred.flatten()))
...
def talker():
    ...
    for time_step in range(0, 1):
        ...
        if time_step == 0:    
            ...
            w_traj_dot = w_traj_dot.reshape(49,-1)
            ...
            w_traj_x = w_traj_x.reshape(49,-1)
        ...
        c_start = time.time()
        css = ddp.solve(xs_pca, us_pca, 30, False)
        c_end = time.time()
