...
class CNN(nn.Module):
    ...
    def forward(self, x):
        out = self.layer3(x)
        out = out.reshape(1, 60, int(self.output_size_/60))
        return out
...
def PCAlearning():
    ...
    if learn_type1 == 0:
        ...
        for key in keys:
            ...
            '''
        '''
        torch.save(x_inputs_train, '/home/jhk/kino_dynamic_learning/dataset/x_inputs_train.pt')
        torch.save(x_inputs_test, '/home/jhk/kino_dynamic_learning/dataset/x_inputs_test.pt')
        torch.save(y_test, '/home/jhk/kino_dynamic_learning/dataset/y_test.pt')
        torch.save(y_vel_test, '/home/jhk/kino_dynamic_learning/dataset/y_vel_test.pt')
        ...
        torch.save(y_x_test, '/home/jhk/kino_dynamic_learning/dataset/y_x_test.pt')
        torch.save(y_train, '/home/jhk/kino_dynamic_learning/dataset/y_train.pt')
        torch.save(y_vel_train, '/home/jhk/kino_dynamic_learning/dataset/y_vel_train.pt')
        ...
        torch.save(y_x_train, '/home/jhk/kino_dynamic_learning/dataset/y_x_train.pt')
        '''
        pickle.dump(x_inputs_train, open('/home/jhk/kino_dynamic_learning/dataset/x_inputs_train.pkl',"wb"))
        pickle.dump(x_inputs_test, open('/home/jhk/kino_dynamic_learning/dataset/x_inputs_test.pkl',"wb"))
        pickle.dump(y_test, open('/home/jhk/kino_dynamic_learning/dataset/y_test.pkl',"wb"))
        pickle.dump(y_vel_test, open('/home/jhk/kino_dynamic_learning/dataset/y_vel_test.pkl',"wb"))
        #pickle.dump(y_u_test, open('/home/jhk/kino_dynamic_learning/dataset/y_u_test.pkl',"wb"))
        #pickle.dump(y_acc_test, open('/home/jhk/kino_dynamic_learning/dataset/y_acc_test.pkl',"wb"))
        pickle.dump(y_x_test, open('/home/jhk/kino_dynamic_learning/dataset/y_x_test.pkl',"wb"))
        pickle.dump(y_train, open('/home/jhk/kino_dynamic_learning/dataset/y_train.pkl',"wb"))
        pickle.dump(y_vel_train, open('/home/jhk/kino_dynamic_learning/dataset/y_vel_train.pkl',"wb"))
        #pickle.dump(y_u_train, open('/home/jhk/kino_dynamic_learning/dataset/y_u_train.pkl',"wb"))
        #pickle.dump(y_acc_train, open('/home/jhk/kino_dynamic_learning/dataset/y_acc_train.pkl',"wb"))
        pickle.dump(y_x_train, open('/home/jhk/kino_dynamic_learning/dataset/y_x_train.pkl',"wb"))
        
        print("transform SAVE")
    else:
        '''
        x_inputs_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset/x_inputs_train.pt')
        x_inputs_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset/x_inputs_test.pt')
        y_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset/y_test.pt')
        y_vel_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset/y_vel_test.pt')
        ...
        y_x_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset/y_x_test.pt')
        y_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset/y_train.pt')
        y_vel_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset/y_vel_train.pt')
        ...
        y_x_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset/y_x_train.pt')
        '''
        '''
        ...
        '''
        x_inputs_train = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/x_inputs_train.pkl',"rb"))
        x_inputs_test = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/x_inputs_test.pkl',"rb"))
        y_test = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/y_test.pkl',"rb"))
        y_vel_test= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/y_vel_test.pkl',"rb"))
        #y_u_test= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/y_u_test.pkl',"rb"))
        #y_acc_test= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/y_acc_test.pkl',"rb"))
        y_x_test= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/y_x_test.pkl',"rb"))
        y_train= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/y_train.pkl',"rb"))
        y_vel_train= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/y_vel_train.pkl',"rb"))
        #y_u_train= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/y_u_train.pkl',"rb"))
        #y_acc_train= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/y_acc_train.pkl',"rb"))
        y_x_train= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset/y_x_train.pkl',"rb"))
    '''
    device = 'cpu'
    ...
    '''
    #q
    #model = LSTM(input_size = 23, hidden_size = 1260, output_size = 21, num_layers = 2, device=device)
    #model1 = LSTM(input_size = 23, hidden_size = 1200, output_size = 20, num_layers = 2, device=device)
    #model2 = LSTM(input_size = 23, hidden_size = 480, output_size = 8, num_layers = 2, device=device)
    ...
    if learn_type == 0:
            ...
            for i, (seq, target) in enumerate(train_loader):
                ...
                if i % 500 == 0:
                    ...
                    print ('Epoch [{}/{}], iter{}, Loss: {:.4f}, Accuracy: {:.4f}' 
                       .format(epoch+1, i, num_epochs, loss.item(), accuracy))
        ...
        #for i in range()
        '''
        for i in range(0, len(x_inputs_test[key])):
            x_inputs_test_temp['Right'][i] = x_inputs_test[key][0][np.newaxis, :, :]
        for i in range(0, len(x_inputs_train[key])):
            x_inputs_train_temp['Right'][i] = x_inputs_train[key][0][np.newaxis, :, :]
        ''' 
        #print(np.shape(x_inputs_train_temp[key]))
        
        #model.add(Dense(60,  input_shape=(1, 23)))
        model.add(keras.layers.RepeatVector(60))
        model.add(LSTM(21, activation='relu', return_sequences=True))
        
        #model1.add(Dense(60,  input_shape=(1, 23)))
        model1.add(keras.layers.RepeatVector(60))
        model1.add(LSTM(20, activation='relu', return_sequences=True))
        #model1.add(Dense(60))
        #model2.add(Dense(60,  input_shape=(1, 23)))
        model2.add(keras.layers.RepeatVector(60))
        ...
        print(np.shape(y_vel_train[key]))
        history = model.fit(x_inputs_train[key], y_train[key], epochs=1, batch_size = 1, validation_data=(x_inputs_test[key], y_test[key]), verbose=1)
        model.summary()
        history1 = model1.fit(x_inputs_train[key], y_vel_train[key], epochs=1, batch_size = 1, validation_data=(x_inputs_test[key], y_vel_test[key]),verbose=1)
        history2 = model2.fit(x_inputs_train[key], y_x_train[key], epochs=1, batch_size = 1, validation_data=(x_inputs_test[key], y_x_test[key]), verbose=1)
        ...
        model = keras.models.load_model('/home/jhk/keras.pkl', compile = True)
        model1 = keras.models.load_model('/home/jhk/keras1.pkl', compile = True)
        model2 = keras.models.load_model('/home/jhk/keras2.pkl', compile = True)
    ...
    ti = time.time()
    traj1 = model.predict(X)
    '''
    c = torch.tensor(X,dtype=torch.float32)
    traj1 = model.forward(c)
    traj1 = traj1[0].detach().numpy()
    w_traj = w_traj[0].detach().numpy()
    w_traj = pca['Right'].inverse_transform([w_traj[None,:]])[0]
    w_traj = w_traj.reshape(rbf_num,-1)
    traj1 = np.dot(Phi,w_traj)
    ...
    '''
    traj1 = model1.predict(X)
    v_traj = traj1[0] #.flatten()
    t2 = time.time()
    acc_traj = np.subtract(traj1[0][1:60,:], traj1[0][0:59,:])/0.02
    t3 = time.time()
    print(t2 - ti)
    print(t3 - ti)
    print(np.shape(acc_traj))

    ti = time.time()
    traj1 = model2.predict(X)
    '''
    a = np.array(X[:])
    c = torch.tensor(a.reshape(1,1,input_size),dtype=torch.float32)
    traj1 = model2.forward(c)
    traj1 = traj1[0].detach().numpy()
    ...
    for i in range(0, 59):
        u_traj[i][0] = (traj1[0][i + 1][2] - traj1[0][i][2])/0.02
        u_traj[i][1] = (traj1[0][i + 1][3] - traj1[0][i][3])/0.02
        u_traj[i][2] = (traj1[0][i + 1][6] - traj1[0][i][6])/0.02
        u_traj[i][3] = (traj1[0][i + 1][7] - traj1[0][i][7])/0.02
    ...
    '''
    q_pca = np.array(trajs[key][KKK]).reshape(60,21)
    v_pca = np.array(vel_trajs[key][KKK]).reshape(60,20)
    x_pca = np.array(x_trajs[key][KKK]).reshape(60,8)
    acc_pca = np.array(acc_trajs[key][KKK]).reshape(59,20)
    u_pca = np.array(u_trajs[key][KKK]).reshape(59,4)
    '''
    xs_pca = []
    us_pca = []
...
def talker():
    ...
    for time1 in range(0, 1):
        ...
        c_start = time.time()
        css = ddp.solve(xs_pca, us_pca, 300, True)
        c_end = time.time()
