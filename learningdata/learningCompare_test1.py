...
class VAE(nn.Module):
    def __init__(self, input_size, output_size, device):
        ...
        self.encoder = VariationalEncoder(input_size, output_size, device)
        self.decoder = Decoder(input_size, output_size, device)
...
class Decoder(nn.Module):
    def __init__(self, input_size, output_size, device):
        ...
        self.linear1 = nn.Linear(output_size, int(input_size * output_size/20))
        self.linear2 = nn.Linear(int(input_size * output_size/20), input_size * output_size)
...
class VariationalEncoder(nn.Module):
    def __init__(self, input_size, output_size, device):
        ...
        self.linear1 = nn.Linear(input_size * output_size, int(input_size * output_size/20))
        self.linear2 = nn.Linear(int(input_size * output_size/20), output_size)
        self.linear3 = nn.Linear(int(input_size * output_size/20), output_size)
...
class AE(models.Model):                                       # AE 모델링 수행하는 역할
    def __init__(self, x_nodes, z_dim):                # x_nodes : 입력 노드수, z_dim : 은닉 노드 수
        ...
        x = layers.Input(shape=x_shape)         #'selu'              # 입력 계층
        z = layers.Dense(int(z_dim* 5), activation='elu')(x)         # 은닉 계층 z_dim에 따라 노드수 정해짐, 활성화 함수는 'ELU'
        z = layers.Dense(int(z_dim* 3), activation='elu')(z)   
        z = layers.Dense(int(z_dim* 1), activation='elu')(z) 

        y = layers.Dense(int(z_dim*1), activation='elu')(z)    # 출력 계층, 활성화 함수는 'sigmoid' (<- 흑백이미지의 특성을 반영하기 위해, 색이 다양하면 선형함수가 더 적합)
        y = layers.Dense(int(z_dim*3), activation='elu')(y) 
        y = layers.Dense(int(z_dim*5), activation='elu')(y) 
        y = layers.Dense(x_nodes, activation='selu')(y) 
...
class AE(torch.nn.Module):
    def __init__(self, input_size, output_size, device):
        ...
        self.encoder = torch.nn.Sequential(
            torch.nn.Linear(input_size * output_size, output_size * 7),
            torch.nn.ELU(),
            torch.nn.Linear(7 * output_size, output_size * 6),
            torch.nn.ELU(),
            torch.nn.Linear(output_size * 6, output_size * 5),
            torch.nn.ELU(),
            torch.nn.Linear(output_size * 5, output_size * 4),
            torch.nn.ELU(),
            torch.nn.Linear(output_size * 4, output_size * 3),
            torch.nn.ELU(),
            torch.nn.Linear(output_size * 3, output_size * 2),
            torch.nn.ELU(),
            torch.nn.Linear(output_size * 2, output_size * 1),
            #torch.nn.ELU()
        )
        ...
        self.decoder = torch.nn.Sequential(
            ...
            torch.nn.ELU(),
            torch.nn.Linear(output_size * 2, output_size * 3),
            torch.nn.ELU(),
            torch.nn.Linear(output_size * 3, output_size * 4),
            torch.nn.ELU(),
            torch.nn.Linear(output_size * 4, output_size * 5),
            torch.nn.ELU(),
            torch.nn.Linear(output_size * 5, output_size * 6),
            torch.nn.ELU(),
            torch.nn.Linear(output_size * 6, output_size * 7),
            torch.nn.ELU(),
            torch.nn.Linear(output_size * 7, int(input_size * output_size)),
            #torch.nn.ELU()
        )
    ...
    def predict(self, x, is_transform = True, learn = 0):
        ...
        if is_transform:
            if (learn == 0):
                ...
                y_transform = self.transform.decoder(torch.tensor(y.reshape(1, -1)))[0]
            elif (learn == 2):
...
def PCAlearning():
    ...
    learn_type2 = 0 #0 PCA 1 kenelPCA 2 LSA
    ...
    if learn_type1 == 0:
        for key in keys:
            ...
            for key in keys:
                ...
                y_u_test[key] = y_x_test[key]
                y_x_test[key] = apply_RBF(y_x_test[key], Phi)
            ...
            torch.save(x_inputs_train, '/home/jhk/kino_dynamic_learning/dataset2/x_inputs_train_train.pt')
            torch.save(x_inputs_test, '/home/jhk/kino_dynamic_learning/dataset2/x_inputs_test_train.pt')
            torch.save(y_test, '/home/jhk/kino_dynamic_learning/dataset2/y_test_train.pt')
            torch.save(y_vel_test, '/home/jhk/kino_dynamic_learning/dataset2/y_vel_test_train.pt')
            torch.save(y_u_test, '/home/jhk/kino_dynamic_learning/dataset2/y_u_test_train.pt')
            torch.save(y_acc_test, '/home/jhk/kino_dynamic_learning/dataset2/y_acc_test_train.pt')
            torch.save(y_x_test, '/home/jhk/kino_dynamic_learning/dataset2/y_x_test_train.pt')
            torch.save(y_train, '/home/jhk/kino_dynamic_learning/dataset2/y_train_train.pt')
            torch.save(y_vel_train, '/home/jhk/kino_dynamic_learning/dataset2/y_vel_train_train.pt')
            torch.save(y_u_train, '/home/jhk/kino_dynamic_learning/dataset2/y_u_train_train.pt')
            torch.save(y_acc_train, '/home/jhk/kino_dynamic_learning/dataset2/y_acc_train_train.pt')
            torch.save(y_x_train, '/home/jhk/kino_dynamic_learning/dataset2/y_x_train_train.pt')

        print("transform SAVE")
    else:
        x_inputs_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset2/x_inputs_train_train.pt')
        x_inputs_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset2/x_inputs_test_train.pt')
        y_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset2/y_test_train.pt')
        y_vel_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset2/y_vel_test_train.pt')
        y_u_test= torch.load( '/home/jhk/kino_dynamic_learning/dataset2/y_u_test_train.pt')
        #y_acc_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset2/y_acc_test.pt')
        y_x_test = torch.load( '/home/jhk/kino_dynamic_learning/dataset2/y_x_test_train.pt')
        y_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset2/y_train_train.pt')
        y_vel_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset2/y_vel_train_train.pt')
        #y_u_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset2/y_u_train.pt')
        #y_acc_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset2/y_acc_train.pt')
        y_x_train = torch.load( '/home/jhk/kino_dynamic_learning/dataset2/y_x_train_train.pt')
        ...
        if load_learning == 0:
            if (learn_type2 == 0):
                for key in keys:
                    ...
                    for epoch in range(num_epochs):
                        losses = []
                        for i in range(0,len(y_x_test[key])): 
                            data = y_x_test[key][i]
                            data = data.detach().numpy()
                            data = data.reshape((1,36*8))
                            #.reshpae(1,288)
                            w = pca_x[key].transform(data)
                            v_pred = pca_x[key].inverse_transform(w)
                            v_pred = v_pred.reshape(36,8)
                            traj1 = np.dot(Phi,v_pred)
                            x_traj = traj1.flatten()
                            norm_eval = traj1 - y_u_test[key][i].detach().numpy().reshape((60,8))
                            
                            a312 = []
                            a313 = []
                            for i in range(0, 60):
                                a312.append([norm_eval[i][0], norm_eval[i][1], norm_eval[i][2], norm_eval[i][4], norm_eval[i][5], norm_eval[i][6]])
                                a313.append([norm_eval[i][3], norm_eval[i][7]])
                            
                            a3123 = np.linalg.norm(a312)
                            a3133 = np.linalg.norm(a313)
                            error_.append([a3123, a3133])
                            
                            '''
                            print(np.linalg.norm(norm_eval.ravel()))
                            batch_loss = loss(y_u_test[key][i], torch.FloatTensor(traj1)) # difference between actual and reconstructed   
                            '''
                            #print(batch_loss.item())
                            #losses.append(batch_loss.item())
                            #running_loss = np.mean(losses)
                            #print(np.shape(norm_eval))
                            #a312 = []
                            #a313 = []
                            #for i in range(0, 60):
                            #    a312.append([norm_eval[i][0], norm_eval[i][1], norm_eval[i][2], norm_eval[i][4], norm_eval[i][5], norm_eval[i][6]])
                           ...
                           # a3123 = np.linalg.norm(norm_eval)
                           # a3133 = np.linalg.norm(a313)
                           # error_.append(a3123)
                ...
                pickle.dump(pca, open('/home/jhk/kino_dynamic_learning/dataset2/w_trajs_pca_train.pkl',"wb"))
                pickle.dump(pca_vel, open('/home/jhk/kino_dynamic_learning/dataset2/w_vel_trajs_pca_train.pkl',"wb"))
                pickle.dump(pca_x, open('/home/jhk/kino_dynamic_learning/dataset2/w_x_trajs_pca_train.pkl',"wb"))
                pickle.dump(pca_u, open('/home/jhk/kino_dynamic_learning/dataset2/w_u_trajs_pca_train.pkl',"wb"))
                pickle.dump(pca_acc, open('/home/jhk/kino_dynamic_learning/dataset2/w_acc_trajs_pca_train.pkl',"wb"))
            
                pickle.dump(w_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_trajs_pca_train.pkl',"wb"))
                pickle.dump(w_vel_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_vel_trajs_pca_train.pkl',"wb"))
                pickle.dump(w_x_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_x_trajs_pca_train.pkl',"wb"))
                pickle.dump(w_u_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_u_trajs_pca_train.pkl',"wb"))
                pickle.dump(w_acc_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_acc_trajs_pca_train.pkl',"wb"))
                ...
                for key in keys:
                    ...
                    pickle.dump(pca, open('/home/jhk/kino_dynamic_learning/dataset2/w_trajs_kernelpca_train.pkl',"wb"))
                    pickle.dump(w_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_trajs_kernelpca_train.pkl',"wb"))
                    '''
                    '''
                    lr = 0.005
                    ...
                    pickle.dump(pca_vel, open('/home/jhk/kino_dynamic_learning/dataset2/w_vel_trajs_kernelpca_train.pkl',"wb"))
                    pickle.dump(w_vel_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_vel_kerneltrajs_pca_train.pkl',"wb"))
                    '''
                    '''
                    lr = 0.001
                    pca[key] = AE(8 *rbf_num, rbf_num, 'cpu')
                    y_train[key] = y_train[key].reshape(10017, 21*rbf_num)
                    y_x_test[key] = y_x_test[key].reshape(1113, 21*rbf_num)
                    history = pca[key].fit(y_train[key].detach().numpy(), y_train[key].detach().numpy(),       # 입력, 출력을 동일하게 설정
                              epochs=2,              # 10회 학습
                              batch_size=5,         # 1회 배치 마다 256개 프로세스 보내도록 설정
                              shuffle=True, 
                              validation_data=(y_x_test[key].detach().numpy(), y_x_test[key].detach().numpy())) # 성능 평가 데이터
                    '''
                    ...
                    for epoch in range(num_epochs):
                        ...
                        for data_list in train_x_loader: 
                            i = i + 1
                            seq, data = data_list
                            data = data.reshape(288) 
                            v_pred = pca_x[key](data) 
                            batch_loss = loss(v_pred, data) # difference between actual and reconstructed   
                            losses.append(batch_loss.item())
                            optimizer.zero_grad()
                            batch_loss.backward()
                            optimizer.step()
                    ...
                    a = 0
                    w_x_trajs_pca[key] = []
                    # print(pytorch_model_summary.summary(model, torch.zeros(8*rbf_num)))
                    error_ = []
                    
                    for data in train_x_loader:
                        a = a + 1
                        seq, target = data
                        data = torch.FloatTensor(target.detach().numpy().reshape(8*rbf_num))
                        w_x_trajs_pca[key].append(pca_x[key].encoder(data).detach().numpy())
                    w_x_trajs_pca[key] = np.array(w_x_trajs_pca[key])
                    ...
                    for data in test_x_loader:
                        #a = a + 1
                        seq, target = data
                        data = target.detach().numpy().reshape(1, 21*rbf_num) 
                        v_pred = pca[key].predict(data) 
                        norm_eval = (v_pred.reshape(rbf_num,21)- target.detach().numpy().reshape(rbf_num,21))#.detach().numpy()
                        
                        a312 = []
                        a313 = []
                        for i in range(0, rbf_num):
                            a312.append([norm_eval[i][0], norm_eval[i][1], norm_eval[i][2], norm_eval[i][4], norm_eval[i][5], norm_eval[i][6]])
                            a313.append([norm_eval[i][3], norm_eval[i][7]])
                        a3123 = np.linalg.norm(a312)
                        a3133 = np.linalg.norm(a313)
                        ...
                        error_.append(np.linalg.norm(norm_eval))
                    print(np.mean(error_, axis=0))
                    '''
                        #w_x_trajs_pca[key] = np.array(w_x_trajs_pca[key]).reshape(a,rbf_num)
                    

                    pickle.dump(pca_x, open('/home/jhk/kino_dynamic_learning/dataset2/w_x_trajs_kernelpca_train.pkl',"wb"))
                    pickle.dump(w_x_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_x_trajs_kernelpca_train.pkl',"wb"))
                    
                pickle.dump(pca_u, open('/home/jhk/kino_dynamic_learning/dataset2/w_u_trajs_kernelpca_train.pkl',"wb"))
                pickle.dump(pca_acc, open('/home/jhk/kino_dynamic_learning/dataset2/w_acc_trajs_kernelpca_train.pkl',"wb"))
                
                pickle.dump(w_u_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_u_trajs_kernelpca_train.pkl',"wb"))
                pickle.dump(w_acc_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_acc_trajs_kernelpca_train.pkl',"wb"))
                ...
                for key in keys:
                   ...
                   # w_u_trajs_pca[key] = pca_u[key].fit_transform(y_u_train[key].detach().numpy())
                pickle.dump(pca, open('/home/jhk/kino_dynamic_learning/dataset2/w_trajs_LSA_train.pkl',"wb"))
                pickle.dump(pca_vel, open('/home/jhk/kino_dynamic_learning/dataset2/w_vel_trajs_LSA_train.pkl',"wb"))
                pickle.dump(pca_x, open('/home/jhk/kino_dynamic_learning/dataset2/w_x_trajs_LSA_train.pkl',"wb"))
                pickle.dump(pca_u, open('/home/jhk/kino_dynamic_learning/dataset2/w_u_trajs_LSA_train.pkl',"wb"))
                pickle.dump(pca_acc, open('/home/jhk/kino_dynamic_learning/dataset2/w_acc_trajs_LSA_train.pkl',"wb"))

                pickle.dump(w_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_trajs_LSA_train.pkl',"wb"))
                pickle.dump(w_vel_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_vel_trajs_LSA_train.pkl',"wb"))
                pickle.dump(w_x_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_x_trajs_LSA_train.pkl',"wb"))
                pickle.dump(w_u_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_u_trajs_LSA_train.pkl',"wb"))
                pickle.dump(w_acc_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset2/w1_acc_trajs_LSA_train.pkl',"wb"))
        else:   
            if (learn_type2 == 0):        
                pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_trajs_pca_train.pkl','rb'))
                pca_vel = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_vel_trajs_pca_train.pkl','rb'))
                pca_x= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_x_trajs_pca_train.pkl','rb'))
                pca_u = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_u_trajs_pca_train.pkl','rb'))
                pca_acc = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_acc_trajs_pca_train.pkl','rb'))

                w_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_trajs_pca_train.pkl','rb'))
                w_vel_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_vel_trajs_pca_train.pkl','rb'))
                w_x_trajs_pca= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_x_trajs_pca_train.pkl','rb'))
                w_u_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_u_trajs_pca_train.pkl','rb'))
                w_acc_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_acc_trajs_pca_train.pkl','rb'))
            elif (learn_type2 == 1):
                pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_trajs_kernelpca_train.pkl','rb'))
                pca_vel = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_vel_trajs_kernelpca_train.pkl','rb'))
                pca_x= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_x_trajs_kernelpca_train.pkl','rb'))
                pca_u = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_u_trajs_kernelpca_train.pkl','rb'))
                pca_acc = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_acc_trajs_kernelpca_train.pkl','rb'))

                w_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_trajs_kernelpca_train.pkl','rb'))
                w_vel_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_vel_trajs_kernelpca_train.pkl','rb'))
                w_x_trajs_pca= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_x_trajs_kernelpca_train.pkl','rb'))
                w_u_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_u_trajs_kernelpca_train.pkl','rb'))
                w_acc_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_acc_trajs_kernelpca_train.pkl','rb'))
            elif (learn_type2 == 2):
                pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_trajs_LSA_train.pkl','rb'))
                pca_vel = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_vel_trajs_LSA_train.pkl','rb'))
                pca_x= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_x_trajs_LSA_train.pkl','rb'))
                pca_u = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_u_trajs_LSA_train.pkl','rb'))
                pca_acc = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w_acc_trajs_LSA_train.pkl','rb'))
                
                w_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_trajs_LSA_train.pkl','rb'))
                w_vel_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_vel_trajs_LSA_train.pkl','rb'))
                w_x_trajs_pca= pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_x_trajs_LSA_train.pkl','rb'))
                w_u_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_u_trajs_LSA_train.pkl','rb'))
                w_acc_trajs_pca = pickle.load(open('/home/jhk/kino_dynamic_learning/dataset2/w1_acc_trajs_LSA_train.pkl','rb'))
    ...
    if(learn_type3 == 0):
        ...
        if test_data == 0:
            for JJ in range(0, len(x_inputs_test[key])):
                ...
                x_traj1 = np.dot(Phi,w_x.detach().numpy())
