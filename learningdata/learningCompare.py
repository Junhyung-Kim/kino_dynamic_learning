...
def PCAlearning():
    ...
    load_learning = 1#data reduction load learning 1
    test_data = 0
    ...
    if learn_type1 == 0:
        for key in keys:
            ...
            #y_acc_train[key] = torch.FloatTensor(c(y_acc_train[key]))
            ...
            if (learn_type2 == 0):
                for key in keys:
                  ...
                  #  w_u_trajs_pca[key] = pca_u[key].fit_transform(y_u_train[key].detach().numpy())
                    n_components = len(pca_x[key].explained_variance_ratio_)
                    explained_variance = pca_x[key].explained_variance_ratio_
                ...
                for key in keys:
                    pca[key] = KernelPCA(n_components = int(rbf_num), fit_inverse_transform=True, kernel ='rbf', gamma = 5, eigen_solver='randomized')
                    w_trajs_pca[key] = pca[key].fit_transform(y_train[key].detach().numpy())
                    
                    pca_vel[key] = KernelPCA(n_components=int(rbf_num), fit_inverse_transform=True, kernel ='rbf', gamma = 5, eigen_solver='randomized')
                    w_vel_trajs_pca[key] = pca_vel[key].fit_transform(y_vel_train[key].detach().numpy())

                    pca_x[key] = KernelPCA(n_components= int(rbf_num), fit_inverse_transform=True, kernel ='rbf', gamma = 5, eigen_solver='randomized')
                    w_x_trajs_pca[key] = pca_x[key].fit_transform(y_x_train[key].detach().numpy())
                ...
                pickle.dump(w_vel_trajs_pca, open('/home/jhk/kino_dynamic_learning/dataset/w1_vel_trajs_kernelpca_train.pkl',"wb"))
                ...
                for key in keys:
                    pca[key] = TruncatedSVD(n_components = int(rbf_num))
                    w_trajs_pca[key] = pca[key].fit_transform(y_train[key].detach().numpy())
                    
                    pca_vel[key] = TruncatedSVD(n_components=int(rbf_num))
                    w_vel_trajs_pca[key] = pca_vel[key].fit_transform(y_vel_train[key].detach().numpy())

                    pca_x[key] = TruncatedSVD(n_components= int(rbf_num))
                    w_x_trajs_pca[key] = pca_x[key].fit_transform(y_x_train[key].detach().numpy())
