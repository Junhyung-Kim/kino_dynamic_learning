...
def talker():
    ...
    p1 = multiprocessing.Process(target=InversePCA, args=(NN_, rbf_num, PCA_, PHI_, tick, queue, thread_manager))
    p2 = multiprocessing.Process(target=InversePCA1, args=(NN_VEL, rbf_num, PCA_VEL, PHI_, tick, queue, thread_manager))
    p3 = multiprocessing.Process(target=InversePCA2, args=(NN_X, rbf_num, PCA_X, PHI_, tick, queue, thread_manager))
    ...
    array_boundx_ = [[] for i in range(len(lines2) + 1)]
    array_boundy_ = [[] for i in range(len(lines2) + 1)]

    array_boundRF = [[] for i in range(int(len(lines1)))]
    array_boundLF = [[] for i in range(int(len(lines1)))]

    array_boundRF_ = [[] for i in range(len(lines1) + 1)]
    array_boundLF_ = [[] for i in range(len(lines1) + 1)]

    zmp_refx = [[] for i in range(len(lines3))]
    zmp_refy = [[] for i in range(len(lines3))]

    zmp_refx_ = [[] for i in range(len(lines3) + 1)]
    zmp_refy_ = [[] for i in range(len(lines3) + 1)]
    ...
    for time_step in range(0, total_time):
        ...
        while True:
            if signal == False:
                statemachinedata[0] = 1
                statemachine.write(statemachinedata)
                x_initv  = x_init.read()
                if time_step == 0:
                    queue[:] = np.ndarray(shape=(23,), dtype=np.float64, buffer=x_initv)
                else:
        ...
        if time_step == 0:
            ...
            x0[41] = xs_pca_test[21]#data.com[0][0]
            x0[43] = xs_pca_test[21]
            x0[45] = xs_pca_test[22]#data.com[0][1]
        ...
        for i in range(0,N-1):
            if time_step == 1:
                time_step1 = 100 #time_step
            else:
                time_step1 = time_step
            state_bounds[i].lb[0] = copy(array_boundx_[i+time_step1][0])
            state_bounds[i].ub[0] = copy(array_boundx_[i+time_step1][1])
            state_bounds[i].lb[1] = copy(array_boundy_[i+time_step1][0])
            state_bounds[i].ub[1] = copy(array_boundy_[i+time_step1][1])
            ...
            rf_foot_pos_vector[i].translation[0] = copy(array_boundRF_[i+time_step1][0])
            rf_foot_pos_vector[i].translation[1] = copy(array_boundRF_[i+time_step1][1])
            rf_foot_pos_vector[i].translation[2] = copy(array_boundRF_[i+time_step1][2])
            lf_foot_pos_vector[i].translation[0] = copy(array_boundLF_[i+time_step1][0])
            lf_foot_pos_vector[i].translation[1] = copy(array_boundLF_[i+time_step1][1])
            lf_foot_pos_vector[i].translation[2] = copy(array_boundLF_[i+time_step1][2])
            residual_FrameRF[i] = crocoddyl.ResidualKinoFramePlacement(state_vector[i], RFframe_id, rf_foot_pos_vector[i], actuation_vector[i].nu + 4)
            residual_FrameLF[i] = crocoddyl.ResidualKinoFramePlacement(state_vector[i], LFframe_id, lf_foot_pos_vector[i], actuation_vector[i].nu + 4)
            foot_trackR[i] = crocoddyl.CostModelResidual(state_vector[i], crocoddyl.ActivationModelWeightedQuad(weight_quad_rf), residual_FrameRF[i])
            foot_trackL[i] = crocoddyl.CostModelResidual(state_vector[i], crocoddyl.ActivationModelWeightedQuad(weight_quad_lf), residual_FrameLF[i])
        ...
        state_bounds[N-1].lb[0] = copy(array_boundx_[N-1+time_step1][0])
        state_bounds[N-1].ub[0] = copy(array_boundx_[N-1+time_step1][1])
        state_bounds[N-1].lb[1] = copy(array_boundy_[N-1+time_step1][0])
        state_bounds[N-1].ub[1] = copy(array_boundy_[N-1+time_step1][1])
        state_activations[N-1].bounds = state_bounds[N-1]
        stateBoundCost_vector[N-1].activation_ = state_activations[N-1]
        rf_foot_pos_vector[N-1].translation[0] = copy(array_boundRF_[N-1+time_step1][0])
        rf_foot_pos_vector[N-1].translation[1] = copy(array_boundRF_[N-1+time_step1][1])
        rf_foot_pos_vector[N-1].translation[2] = copy(array_boundRF_[N-1+time_step1][2])
        lf_foot_pos_vector[N-1].translation[0] = copy(array_boundLF_[N-1+time_step1][0])
        lf_foot_pos_vector[N-1].translation[1] = copy(array_boundLF_[N-1+time_step1][1])
        lf_foot_pos_vector[N-1].translation[2] = copy(array_boundLF_[N-1+time_step1][2])
        residual_FrameRF[N-1] = crocoddyl.ResidualKinoFramePlacement(state_vector[N-1], RFframe_id, rf_foot_pos_vector[N-1], actuation_vector[N-1].nu + 4)
        residual_FrameLF[N-1] = crocoddyl.ResidualKinoFramePlacement(state_vector[N-1], LFframe_id, lf_foot_pos_vector[N-1], actuation_vector[N-1].nu + 4)
        foot_trackR[N-1] = crocoddyl.CostModelResidual(state_vector[N-1], crocoddyl.ActivationModelWeightedQuad(weight_quad_rf), residual_FrameRF[N-1])
        foot_trackL[N-1] = crocoddyl.CostModelResidual(state_vector[N-1], crocoddyl.ActivationModelWeightedQuad(weight_quad_lf), residual_FrameLF[N-1])    
