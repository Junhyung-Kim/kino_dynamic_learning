...
def talker():
    ...
    with open('/home/jhk/ssd_mount/afterprocessing/upperbody/integral/filename1.pkl', 'rb') as f:
        database = pickle.load(f,  encoding='iso-8859-1')
    f.close()
    ...
    e = 0
    for time1 in range(90, len(database['Right']['u_trajs'])):#15, len(database['Right']['trajs'])):
        if(database['Right']['costs'][time1] > 0.01) and time1 != 51:
            for i in range(0, len(q)-2):    
                q[i] = database['Right']['trajs'][time1][28][i]
            ...
            weight_quad_zmp1 = np.array([2.0, 8.0]) ##11
            weight_quad_cam = np.array([0.005, 0.005])#([weight_quad_camy] + [weight_quad_camx])
            weight_quad_upper = np.array([0.8, 0.8])
            weight_quad_com = np.array([0.80, 0.80, 0.80])#([weight_quad_comx] + [weight_quad_comy] + [weight_quad_comz])
            weight_quad_rf = np.array([0.80, 0.80, 0.80, 0.20, 0.20, 0.20])#np.array([weight_quad_rfx] + [weight_quad_rfy] + [weight_quad_rfz] + [weight_quad_rfroll] + [weight_quad_rfpitch] + [weight_quad_rfyaw])
            weight_quad_lf = np.array([0.80, 0.80, 0.80, 0.20, 0.20, 0.20])#np.array([weight_quad_lfx] + [weight_quad_lfy] + [weight_quad_lfz] + [weight_quad_lfroll] + [weight_quad_lfpitch] + [weight_quad_lfyaw])
            ...
            for i in range(0,N):
                xs[i] = np.append(np.append(database['Right']['trajs'][time1][i], database['Right']['vel_trajs'][time1][i]),database['Right']['x_state'][time1][i])
            
            for i in range(0,N-1):
                us[i] = np.append(database['Right']['u_trajs'][time1][i],database['Right']['acc_trajs'][time1][i])#np.append(np.append(database['Right']['u_trajs'][time1][i+1], database['Right']['acc_trajs'][time1][i][0]), database['Right']['acc_trajs'][time1][i][2])
            ...
            while booltemp == True:
                ...
                c_start = time.time()
                if iter_ == 0:
                    css = ddp.solve(xs, us, 10000, False)
                else:
                    css = ddp.solve(ddp.xs, ddp.us, 100000,  False)
                c_end = time.time()
                ...
                if booltemp1 == False:
                    ...
                    database[key]['u_trajs'][time1] = (copy(acc_traj))
                    database[key]['acc_trajs'][time1] = (copy(u_traj))

                    database['Right']['x_inputs'][time1] = (copy(ddp.xs[0][0:21]))
                    database['Right']['vel_trajs'][time1] = (copy(vel_traj))
                    database['Right']['x_state'][time1] = (copy(x_traj))
                    database['Right']['costs'][time1] = (copy(ddp.cost))
                    database['Right']['iters'][time1] = (copy(ddp.iter))
                    database['Right']['trajs'][time1] = (copy(traj))
                    database['Right']['data_phases_set'][time1] = ([css, avrg_duration])
                
                    with open('/home/jhk/ssd_mount/afterprocessing/upperbody/integral/filename5.pkl', 'wb') as f:
