...
def talker():
    ...
    for time_step in range(0, 1):
        ...
        with open('/home/jhk/ssd_mount/beforedata/ssp1/i=0/filename4_0.pkl', 'rb') as f:
            database = pickle.load(f,  encoding='iso-8859-1')
        f.close()
        ...
        for time1 in range(0, len(database_['Right']['u_trajs'])):
            ...
            print(data.com[0])
            for i1 in range(1,2):
                for j1 in range(1, 2): #4
                    print("vel")
                    print([time1, i1, j1])
                    print(database_['Right']['vel_trajs'][time1][0][0])
                    PELV_move = [-database_['Right']['vel_trajs'][time1][0][0] *0.02 -0.002*(j1-2.5), -database_['Right']['vel_trajs'][time1][0][1] *0.02 -0.001*(j1-2.5), 0.0] 
                    ...
                    while booltemp == True:
                        ...
                        if booltemp1 == False:
                            if iter_ >= 30 or abs(ddp.cost - cost_prev) < 0.000001 or ddp.cost < 0.001:
                                booltemp = False
                            cost_prev = ddp.cost
                            if iter_ == 0:
                                for key in crocs_data.keys():
                                ...
                                crocs_data['Right']['time'].append([i1, j1, -0.002*(j1-2.5) -0.001*(j1-2.5)])
                            else:
                                traj = np.array(ddp.xs)[:,0:21]
                                vel_traj = np.array(ddp.xs)[:,21:41]
                                x_traj = np.array(ddp.xs)[:, 41:49]
                                ...
                                crocs_data['Right']['time'][e] = [i1, j1, -0.002*(j1-2.5) -0.001*(j1-2.5)]
                        ...
                        if i1 == 0 and j1 == 0:
                            file_name ='/home/jhk/ssd_mount/beforedata/ssp1/i=0/filename4_11_dist_'
                            file_name2 = '.pkl'
                            file_name3 = file_name + str(time_step) + file_name2
