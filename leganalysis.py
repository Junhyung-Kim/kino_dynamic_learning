...
def loadmodel(kkkk):
    ...
    filename = '/home/jhk/ssd_mount'
    #filename1 =  str(kkkk)   
    filename2 = '/Fdyn_data5_9.txt'
    filename3 = filename + filename2 # + filename2
...
def talker():
    ...
    for kkkk in range(8,9):
        ...
        N = 71
        k = 1
        f = open("/home/jhk/ssd_mount/lfoot2_3_1.txt", 'r')
        f1 = open("/home/jhk/ssd_mount/rfoot2_3_1.txt", 'r')
        lines1 = f1.readlines()
        lines = f.readlines()
        ...
        for time1 in range(0,  len(database1['Right']['vel_trajs'])):
            ...
            for i in range(0, N):
                if LIPM_bool == 1:
                    ...
                    hg = [database['Right']['x_state'][time1][i][7], database['Right']['x_state'][time1][i][3]]
                    
                    com_refd = com_refd + [0.0- 2.0 *(data.com[0][0] -comrefx), - 5.0 *(data.com[0][1] -comrefy), 0.0]
                    #om_refd = com_refd + [0.0- 2.0 *(data.com[0][0] -array_boundRF_[i][0]), 0.0, - 15.0 *(data.oMf[LFframe_id].translation[2] -array_boundLF_[i][2])]
                    ...
                    RF_d = RF_d + [0.0- 10.0 *(data.oMf[RFframe_id].translation[0] -array_boundRF_[i][0]), 0.0, - 15.0 *(data.oMf[RFframe_id].translation[2] -array_boundRF_[i][2])]
                    LF_d = LF_d + [0.0- 10.0 *(data.oMf[LFframe_id].translation[0] -array_boundLF_[i][0]), 0.0, - 15.0 *(data.oMf[LFframe_id].translation[2] -array_boundLF_[i][2])]
                    ...
                    if phase_variable[i] != 1:
                        ...
                        if phase_variable[i] == 2:
                            ...
                            temp1[0] = data.oMf[RFcframe_id].translation[0] + (-sol[i][4])/sol[i][2]
                            temp1[1] = data.oMf[RFcframe_id].translation[1] + (sol[i][3])/sol[i][2]
                            ...
                            temp[0:6] = sol[i][0:6]
                            contact_wrench.append(temp)   
                            temp1[0] = data.oMf[LFcframe_id].translation[0] + (-sol[i][4])/sol[i][2]
                            temp1[1] = data.oMf[LFcframe_id].translation[1] + (sol[i][3])/sol[i][2]
                            zmp_est.append(temp1)   
                    else:
                        ...
                        temp1[1] = zmp_refy[i]
                        zmp_est.append(temp1)
                    zmp_err.append([zmp_est[i][0] - zmp_refx[i], zmp_est[i][1] - zmp_refy[i]])
                    if time1 == 0:
                        print("Ddd")
                        f3.write('%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f' % (i, data.com[0][0], data.com[0][1], comrefx, comrefy, zmp_refx[i], zmp_refy[i], zmp_est[i][0], zmp_est[i][1], data.hg.angular[0], data.hg.angular[1], database['Right']['x_state'][time1][i][7], database['Right']['x_state'][time1][i][3], data.oMf[RFcframe_id].translation[0], data.oMf[LFcframe_id].translation[0], array_boundRF[i][0], array_boundLF[i][0]))
                        f3.write("\n")
                        print(q)
                    '''
                else:
