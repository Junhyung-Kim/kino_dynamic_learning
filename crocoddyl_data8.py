...
def PCAlearning():
    ...
    with open('/home/jhk/ssd_mount/kdyn_data5.txt', 'rb') as f:
...
def talker():
    ...
    if data_processing == True:
        ...
        for i in range(0, len(database[key]['trajs'])):
            ...
            vel_temp = []
            u_temp = []
            for j in range(0 + step_time, N + step_time):
                ...
                if j != N-1:
                    ...
                    u_temp.append(database['Right']['u_trajs'][i][k1*j+1])
                if j == N-1:
    ...
    e = 0
    for time1 in range(10, 11):#len(database_['Right']['u_trajs'])):#15, len(database_['Right']['trajs'])):
        ...
        weight_quad_upper = np.array([1.2, 1.2])
        weight_quad_com = np.array([1.0, 1.0, 1.0])#([weight_quad_comx] + [weight_quad_comy] + [weight_quad_comz])
        weight_quad_rf = np.array([1.0, 1.0, 1.0, 0.5, 0.5, 0.5])#np.array([weight_quad_rfx] + [weight_quad_rfy] + [weight_quad_rfz] + [weight_quad_rfroll] + [weight_quad_rfpitch] + [weight_quad_rfyaw])
        weight_quad_lf = np.array([1.0, 1.0, 1.0, 0.5, 0.5, 0.5])#np.array([weight_quad_lfx] + [weight_quad_lfy] + [weight_quad_lfz] + [weight_quad_lfroll] + [weight_quad_lfpitch] + [weight_quad_lfyaw])
        ...
        for i in range(0,N-1):
            traj_[43] = (array_boundx_[i + step_time][0] + array_boundx_[i + step_time][1])/2 #zmp_refx_[i][0]
            traj_[47] = (array_boundy_[i + step_time][0] + array_boundy_[i + step_time][1])/2#zmp_refy_[i][0]
        ...
        traj_[43] = zmp_refx_[N-1 + step_time][0]
        traj_[47] = zmp_refy_[N-1 + step_time][0]
        ...
        for i in range(0,N-1):
            ...
            state_bounds[i].lb[0] = copy(array_boundx_[i+ step_time][0])
            state_bounds[i].ub[0] = copy(array_boundx_[i+ step_time][1])
            state_bounds[i].lb[1] = copy(array_boundy_[i+ step_time][0])
            state_bounds[i].ub[1] = copy(array_boundy_[i+ step_time][1])
            ...
            rf_foot_pos_vector[i].translation[0] = copy(array_boundRF_[i+ step_time][0])
            rf_foot_pos_vector[i].translation[1] = copy(array_boundRF_[i+ step_time][1])
            rf_foot_pos_vector[i].translation[2] = copy(array_boundRF_[i+ step_time][2])
            lf_foot_pos_vector[i].translation[0] = copy(array_boundLF_[i+ step_time][0])
            lf_foot_pos_vector[i].translation[1] = copy(array_boundLF_[i+ step_time][1])
            lf_foot_pos_vector[i].translation[2] = copy(array_boundLF_[i+ step_time][2])
            residual_FrameRF[i] = crocoddyl.ResidualKinoFramePlacement(state_vector[i], RFframe_id, rf_foot_pos_vector[i], actuation_vector[i].nu + 4)
            residual_FrameLF[i] = crocoddyl.ResidualKinoFramePlacement(state_vector[i], LFframe_id, lf_foot_pos_vector[i], actuation_vector[i].nu + 4)
            foot_trackR[i] = crocoddyl.CostModelResidual(state_vector[i], crocoddyl.ActivationModelWeightedQuad(weight_quad_rf), residual_FrameRF[i])
            foot_trackL[i] = crocoddyl.CostModelResidual(state_vector[i], crocoddyl.ActivationModelWeightedQuad(weight_quad_lf), residual_FrameLF[i])
        ...
        state_bounds[N-1].lb[0] = copy(array_boundx_[N-1+ step_time][0])
        state_bounds[N-1].ub[0] = copy(array_boundx_[N-1+ step_time][1])
        state_bounds[N-1].lb[1] = copy(array_boundy_[N-1+ step_time][0])
        state_bounds[N-1].ub[1] = copy(array_boundy_[N-1+ step_time][1])
        state_activations[N-1].bounds = state_bounds[N-1]
        stateBoundCost_vector[N-1].activation_ = state_activations[N-1+ step_time]
        rf_foot_pos_vector[N-1].translation[0] = copy(array_boundRF_[N-1+ step_time][0])
        rf_foot_pos_vector[N-1].translation[1] = copy(array_boundRF_[N-1+ step_time][1])
        rf_foot_pos_vector[N-1].translation[2] = copy(array_boundRF_[N-1+ step_time][2])
        lf_foot_pos_vector[N-1].translation[0] = copy(array_boundLF_[N-1+ step_time][0])
        lf_foot_pos_vector[N-1].translation[1] = copy(array_boundLF_[N-1+ step_time][1])
        lf_foot_pos_vector[N-1].translation[2] = copy(array_boundLF_[N-1+ step_time][2])
        residual_FrameRF[N-1] = crocoddyl.ResidualKinoFramePlacement(state_vector[N-1], RFframe_id, rf_foot_pos_vector[N-1], actuation_vector[N-1].nu + 4)
        residual_FrameLF[N-1] = crocoddyl.ResidualKinoFramePlacement(state_vector[N-1], LFframe_id, lf_foot_pos_vector[N-1], actuation_vector[N-1].nu + 4)
        foot_trackR[N-1] = crocoddyl.CostModelResidual(state_vector[N-1], crocoddyl.ActivationModelWeightedQuad(weight_quad_rf), residual_FrameRF[N-1])
        foot_trackL[N-1] = crocoddyl.CostModelResidual(state_vector[N-1], crocoddyl.ActivationModelWeightedQuad(weight_quad_lf), residual_FrameLF[N-1])    
        ...
        while booltemp == True:
            ...
            c_start = time.time()
            if iter_ == 0:
                css = ddp.solve(xs, us, 1, False, 0.0001)##22
            else:
                ddp.th_stop = 0.000001
                #css = ddp.solve(xs, us, 3000, False, 0.0001)##22
            c_end = time.time()
            ...
            if booltemp1 == False:
                if iter_ >= 0 and abs(ddp.cost - cost_prev) < 0.000001 and css == True:
                    booltemp = False
                cost_prev = ddp.cost
                if iter_ == 0:
                    for key in crocs_data.keys():
                ...
                with open('/home/jhk/ssd_mount/beforedata/integrate_fdyn/filename3.pkl', 'wb') as f:
	                pickle.dump(crocs_data, f, protocol=pickle.HIGHEST_PROTOCOL)
                print("success")
            print(database['Right']['x_state'][time1][0])
            print(database['Right']['x_state'][time1][1])
            print(ddp.xs[0][0:21])
            iter_ = iter_ + 1
            if booltemp == False:
                e = e+1
