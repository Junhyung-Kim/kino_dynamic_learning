...
def talker():
    ...
    with open('/home/jhk/ssd_mount/afterprocessing/1/Fdyn_data5.txt', 'rb') as f:
        database = pickle.load(f,  encoding='iso-8859-1')
    f.close()
    ...
    e = 0
    for time1 in range(10, len(database_['Right']['u_trajs'])):#15, len(database_['Right']['trajs'])):
        ...
        while booltemp == True:
            ...
            c_start = time.time()
            if iter_ == 0:
                css = ddp.solve(xs, us, 20000, True)##22
            else:
                ddp.th_stop = 0.00001
                css = ddp.solve(ddp.xs, ddp.us, 20000, True)
            c_end = time.time()
            ...
            if booltemp1 == False:
                if iter_ >= 0 and abs(ddp.cost - cost_prev) < 0.000001:
                    booltemp = False
                cost_prev = ddp.cost
                if iter_ == 0:
                    for key in crocs_data.keys():
                ...
                with open('/home/jhk/ssd_mount/afterprocessing/upperbody/1/filename3.pkl', 'wb') as f:
