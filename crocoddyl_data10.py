...
def talker():
    ...
    #with open('/home/jhk/ssd_mount/afterdata/i=0-8, j=13/Fdyn_data5.txt', 'rb') as f:
        database = pickle.load(f,  encoding='iso-8859-1')
    f.close()
    ...
    for time1 in range(0, len(database_['Right']['u_trajs'])):#15, len(database_['Right']['trajs'])):
        ...
        while booltemp == True:
            ...
            if iter_ % 2 == 0:
                ...
                print("a")
                if iter_ == 0:
                    css = ddp.solve(xs, us, 1000, False, 0.00007)
            else:
                print("b")
                css = ddp.solve(ddp.xs, ddp.us, 1000, False, 0.00007)
            c_end = time.time()
