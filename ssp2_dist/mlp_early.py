...
"timestep=14_finish_ssp2",  
    ...
    file_name2 = "/timestep="
    file_name3 = filename +param[time_step]#+'_ssp2'
    print(file_name3)
    ...
    if learn_type == 0:
        ...
        file_name3 = file_name +'Earlymlp_loss'+ param[time_step] +'.txt'
...
def talker():
    ...
    for i in range(0, 51, 1):#learning_data_num):
        PCAlearning(i)
    k = adsfasdff
    print("start")
    ...
    zmp_ssp2fx = [[] for i in range(len(lines3))]
    zmp_ssp2fy = [[] for i in range(len(lines3))]

    zmp_ssp2fx_ = [[] for i in range(N)]
    zmp_ssp2fy_ = [[] for i in range(N)]
    ...
    for i in range(0, len(lines3_array)):
        for j in range(0, len(lines3_array[i])):
            if j == 0:
                zmp_ssp2fx[i].append(float(lines3_array[i][j]))
            if j == 1:
                zmp_ssp2fy[i].append(float(lines3_array[i][j]))
           
    for i in range(0, N):
        if i == 0:
            zmp_ssp2fx_[i] = zmp_ssp2fx[k*i + time_step]
            zmp_ssp2fy_[i] = zmp_ssp2fy[k*i + time_step]
        else:
            zmp_ssp2fx_[i] = zmp_ssp2fx[k*(i)+ time_step]
            zmp_ssp2fy_[i] = zmp_ssp2fy[k*(i)+ time_step]
 ...
 -0.34694619, -0.06874684, -0.26307327 , 0.04668285 , 0.01114374 , 0.08497735,
  ...
  0.98146212]
        ...
        for i in range(0,N-1):
            traj_[43] = (array_boundx_[i][0] + array_boundx_[i][1])/2 #zmp_ssp2fx_[i][0]
            traj_[47] = (array_boundy_[i][0] + array_boundy_[i][1])/2#zmp_ssp2fy_[i][0]
        ...
        traj_[43] = zmp_ssp2fx_[N-1][0]
        traj_[47] = zmp_ssp2fy_[N-1][0]
